...
- **The Layer-5 read gate is scoped by corpus class, not applied blanket (Phase 133).** The k-anonymity gate on individual-article reads (`GET /articles/{id}`) counts the distinct articles sharing the article's *(source, metric, UTC-day)* aggregation key and refuses below *k*. This gate is **conditional on the re-identification risk of the corpus**, because the threat model it addresses — singling out a private author — does not exist uniformly. A **public institutional publisher** (corpus class `institutional-web`: a government press office, a newsroom with a public canonical article URL) is **exempt** (effective *k* = 1): the article text is already public, and authorship is institutional, not individual, so withholding it protects no one while suppressing legitimate reading of public discourse. Worse, a blanket gate penalises exactly the low-cadence institutional sources whose publication rhythm is itself a finding. The gate therefore stays in **full force** for corpus classes where individuals could be deanonymised (social media, user-generated content), and is relaxed only for classes on the public-institutional allowlist. The same risk asymmetry already governs entity anonymization (§7.2.3) and aggregation windows (§7.2.2 above); this makes it explicit for the read gate. The allowlist is the architectural enforcement point, reviewed as new corpus classes are onboarded.
