...
const FILE_LENGTH_ALLOWLIST = [
  ...
  ['**/PanelCellGrid.svelte', 539], // markup-dominated; Phase 144 facet-disclosure note + Task-B fieldLabel relabels + Phase 148g cross-probe merge props (probeIds/sourceIds/maxNodes + provenanceBorder) threaded to both co-occurrence renderers + guided-tour first-cell hook — within the ±20-30 tolerance band
  ...
  ['**/url-types.ts', 615], // URL state SoT — grows with each lever; guided tour added the `guide` launch field (+ EMPTY_URL_STATE default); Phase 149 added the `about` overlay field (+ EMPTY_URL_STATE default); Phase 148g added maxNodes + showLabels + provenanceBorder (+ compact pv keys); Phase 148e added defaultCompositionForView + initialLeversForView (the create-mode + pillar-seed SoT for co-occurrence opening merged with legible label/settle defaults)
