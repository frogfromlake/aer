...
// a programmatic value is still accepted): bins 1–200, topN 1–max, force 0–100.
export function clampBins(raw: number): number | null {
  if (!Number.isFinite(raw)) return null;
  return Math.min(200, Math.max(1, Math.round(raw)));
}

// topN's ceiling is view-dependent (see computeTopNMax): co-occurrence accepts up
// to 6000 edges, metadata-field views clamp to 200 server-side, all others 500.
// The caller passes the per-view max so the popover clamps to the SAME bound the
// panel-level slider + the BFF enforce. Defaults to the co-occurrence ceiling for
// back-compatible callers / the unit test.
export function clampTopN(raw: number, max = 6000): number | null {
  if (!Number.isFinite(raw)) return null;
