...
* [x] **Makefile targets.** New `make crawl-probe0` target invokes the web-crawler service with `--probe probe0`. Pattern is `make crawl-<probe-id>` for any probe; `make crawl` is retained as a deprecated alias for one release cycle, then retired in Phase 131.
...
* [x] **`docs/extending/add-a-probe.md`** — **UPDATED.** Reflect the new `crawlers/web-crawler/probes/<probe-id>/` configuration pattern. Reference Phase 125's Probe 1 (FR institutional web) as the next worked example. Cross-references `add-a-source.md`.
...
* [x] **`internal/discovery/sitemap.py`.** `discover()` accepts `since: Optional[datetime] = None`. Entries with `sitemap_lastmod < since` are skipped. Entries whose `sitemap_lastmod` is `None` fall through (the worker's existing `timestamp_source = "fetch_at_fallback"` already classifies them as Negative Space per Brief §7.7 — we do not silently drop coverage on publishers with sparse sitemaps).
...
* [x] **`main.py.cli()`.** Logs a single `crawl_window_configured` structlog line per probe at startup (`probe=probe0, time_window_days=1825, since=2021-05-09T00:00:00Z`) — the operations playbook references this line for spot-check verification.
...
* [x] **`docs/methodology/en/WP-001-en-...md` § culturally-agnostic taxonomy.** One-sentence cross-reference from the §5.3 Coverage Map discussion noting that Probe Coverage Maps (Phase 125a) inherit a uniform temporal horizon from the probe config.
