...
## Phase 144: UI Localization — German (DE) [P1] - [~] CORE IMPLEMENTED 2026-06-18 (all 13 UI surfaces localized + mechanism + content/WP wiring green; conceptual-vocabulary SoT tranche remaining — see Coverage)
...
* [x] **All UI-shell strings → Paraglide** — DONE for every reachable surface: chrome (SideRail/ScopeBar/PillarSwitch + pillar questions/plain-language), Atmosphäre (+ WebGL fallback), Workbench (panels/shells/ScopeEditor/PanelControls + all `levers/*` + all presentation `cells/*`), Dossier, auth, Account/Admin, Reflection (+ WP reader/landing/primer/probe/metric), source+article, base primitives (Badge/FunctionBadge/NegativeSpaceBadge/Dialog/SidePanel/DateRangePicker/SkipLink) + RefusalSurface. Includes aria/title/placeholder; pluralised via `_one`/`_other`. **Remaining (conceptual-vocabulary SoT tranche — see note below):** the shared **data SoTs** still hold English `label`/`description`: `presentations/registry.ts` (presentation + pillar labels/descriptions/blurbs, shown in the View picker + tile tooltips), `discourse-function.ts` (4 function labels/descriptions in FunctionBadge), `negative-space.ts` (NS-class labels/descriptions in NegativeSpaceBadge), `methodology-copy.ts` (6 banner templates on 5 chart cells), and the `UNCATEGORISED_LABEL` constant in `topic-internals.ts`/`topic-evolution-internals.ts`. `open-questions.ts` (743-line research-question DATA prose) + `papers.ts` structural `shortTitle` English fallback are out of scope by design.
...
### Validation
* [x] Every converted surface renders DE+EN via the SideRail selector; the **parity gate** (`check-i18n-parity.mjs`, 1270/1270) + Paraglide compile fail on any en-only/de-only or stub message; `?lang=` round-trips + deep-links; date/number follow locale; WP bodies + editorial prose appear in DE; emic nouns untranslated. `make fe-lint` / `fe-test` / SSG build / bundle-size green; Go BFF content config+handler tests green. Playwright `tests/e2e/locale.spec.ts` added (DE render + live switch + `?lang=de` deep-link; runs in CI). **Remaining:** the conceptual-vocabulary SoT tranche above; once localised, the DoD "no hardcoded English in any user-visible surface" is fully met. Recommended recipe (Phase 144b): give each SoT accessor (`getPresentation`/`getPillar`/`getFunctionDef`/`getNSClassDef`/`methodologyNotes`) a locale-aware path — either a `loc` param threaded from `locale()` at the (few) display call-sites, or a **relative** `import { m } from '../paraglide/messages.js'` inside the SoT module (node-test-safe; resolves to EN under the baseLocale strategy so existing English-asserting unit tests still pass) — plus a new `messages/{en,de}/domain.json`.
