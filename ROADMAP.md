...
* [ ] **Manual / DEFERRED to Phase 123.** A cross-source aggregation that touches `author` (e.g. authorship-concentration metric) refuses to compose tagesschau + bundesregierung without surfacing the asymmetry — same gating pattern the cross-frame equivalence gate uses for normalization (Phase 115). Deferred because AĒR has no metric that touches `author` today; the consuming metric is Phase 123's stylometric authenticity work. Wiring a generic field-asymmetry refusal hook now without a consumer is speculative infrastructure (CLAUDE.md "no overengineering"). The data substrate (`structurallyAbsent` flag on the BFF endpoint) is in place; the gate is wired in the phase that introduces the consuming metric.
...
*Question: Since the Crawl window is 7 days for the webcrawler, we could also think about storing each article again next to the other one IF it contains any differences to the first one. This could happen within this 7 days window. Each difference is stored as a revision and the rest of the article is discarded, so we would store 7 days revision for each article without the wayback machine at all.*
...
*Note: Cross check with methodology papers necessary. If e.g. tagesschau is a epistemic_authority, a sports article has its own meaning WITHIN the context of being published by a news outlet flagged as epistemic_authority. This would be PART of the discourse function and underlines the meaning of a sports article within the target group/discourse of this source. If we decide this, the phase could be obsolete. Careful here. This needs to be grounded and decided within the context of the scientific disciplines the workingpapers cover, not a technical decision.*
