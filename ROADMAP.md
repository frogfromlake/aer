...
> **Carried from Phase 149** (deliberate residuals — not part of the live-deploy push). *(Transactional-email live-validation was the third; ✅ DONE 2026-06-27 — closed in Phase 149.)* **(a) GHCR service-image publishing + CD pipeline.** **DONE so far (2026-06-27):** **M1** `release-images.yml` publishes the 4 service images to GHCR on a `v*` tag (matrix; worker uses a registry cache for the model layer; OpenAPI bundle generated in-CI); **M2** `compose.prod.yaml` pulls `image:` + `!reset`s `build:` (GHCR_OWNER/IMAGE_TAG in `.env`); **M3a** box has a read-only `read:packages` token (`docker login ghcr.io`), packages PRIVATE. **B chosen** for secrets-at-rest (keep hardened `.env`; full no-`.env` fix → Phase 155). **REMAINING:** **M4** automate the box-side deploy (GH Actions SSH → `git pull` + `docker compose pull` + `up`) — until then the deploy is *manual but pull-based* (no more building on the box); **M5** un-park the nightly smoke (flip `e2e_smoke_nightly.yml` to pull, validate `e2e_smoke_test.sh` — already a synthetic ingest, not the dead RSS crawler, re-enable cron); **M6** rollback doc (tag-pin + restore); **M7** `deploy_runbook.md` + arc42 deployment-view + Phase-151 drift check. ~~(b) frontend manual passes~~ **✅ DONE 2026-06-27** (live pass on deployed HEAD: disclosure dim, globe fluid + reduced-motion, keyboard-navigable). **(c) [quality] `AdminPanel.svelte` surfaces 403/errors** instead of silently no-op'ing (`resetFor`/`suspend`/`reactivate` swallow non-ok results — found 2026-06-27 during the email test: a stale-admin tab's clicks silently failed; small UX fix, ~3 handlers + an `AuthNotice`).
...
* [ ] **Dedicated datastore exporters (deferred from Phase-149 SR-6 / SEC-052 remainder, operator decision 2026-06-26).** Phase 149 Batch A shipped the footprint-independent host signals (node-exporter disk/mem, generic `TargetDown`, Traefik cert-expiry). Deferred to here as post-launch defense-in-depth: a **postgres_exporter** (connection-pool saturation early-warning before logins fail), a **prometheus-nats-exporter** (`:8222` /varz → Prometheus), and **cadvisor** (per-container OOM / restart-loop). Until then PG/NATS failure is caught indirectly via `TargetDown` on bff/ingestion/worker. Each is a small container; the postgres exporter needs a read-only DSN secret. Add with the rest of the live-guardrail alerting.
