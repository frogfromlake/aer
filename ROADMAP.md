...
### Mechanism
* [ ] **Paraglide/inlang setup** — add `@inlang/paraglide-js` + Vite plugin; `project.inlang` with `baseLocale: en`, locales `[en, de]`. Organise messages **per-feature** (e.g. `messages/{en,de}/{common,chrome,atmosphere,workbench,dossier,auth,account,reflection,errors}.json`, or feature-prefixed keys — pick the inlang messageFormat layout that yields per-feature files; the binding requirement is per-feature maintainability + a parity-lint, not the exact file count). Confirm the SSG build is clean.
* [ ] **Single locale rune** — replace the `APP_CONTENT_LANGUAGE` constant with a `locale` rune store (SoT). Resolution `?lang=` → `localStorage` → `navigator.language`∈{en,de} → `en`; feed Paraglide via its overwrite-get-locale hook (NO cookie strategy). On switch: update rune + localStorage + URL `?lang=`, and put locale in TanStack queryKeys so content refetches.
* [ ] **Locale selector** — in the **SideRail user section** (account area). Persists per the resolution order; sensible default; keyboard-operable + narrated.

### Coverage (every detail)
* [ ] **All UI-shell strings → Paraglide** — chrome, Atmosphäre (incl. WebGL fallback), Workbench (levers/panels/ScopeEditor/PanelControls/MethodologyBanner), Dossier overlay, auth (login/invite/forgot/reset), Account/Admin overlays, Reflection landing, base primitives. **Includes aria-labels, title attributes, placeholders.** Parameterised + pluralised messages for the dynamic-count copy now in `methodology-copy.ts`; localise the `negative-space.ts` labels/descriptions (keep the stable class keys/abbrs).
* [ ] **Editorial content wired to locale** — `contentQuery` + every call-site read the locale rune; implement BFF missing-locale behaviour (recommended: EN-fallback with a logged warning) **and a CI parity gate** so a gap is caught, never silently served as English.
* [ ] **Working Papers per-locale** — serve `docs/methodology/{en,de}/` WP markdown by active locale (documented sync into `static/content/papers/{en,de}/`); reflection reader + landing + per-paper titles/abstracts localised.
* [ ] **Locale-aware formatting** — replace hardcoded `en-CA` date formatters with `Intl.DateTimeFormat(locale)`; route number/relative formats through a `$lib/localization/format.ts` reading the locale rune.
* [ ] **Emic proper nouns stay untranslated** — verify probe/source display-name layers are not routed through messages.

### Validation
* [ ] Every reachable surface renders fully in DE and EN via the SideRail selector; **no hardcoded English remains in a user-visible surface** (a grep/lint gate for raw Svelte text + inlang parity-lint failing on any en-only/de-only message); `?lang=` round-trips and deep-links reproduce language; date/number formats follow locale; WP bodies + editorial prose appear in DE; emic proper nouns stay untranslated. `make lint` / `fe-lint` / `test` green; sets up Phase 128's EN/DE-parity audit.
