...
## Phase 149: Provisioning & First Deploy [P0] - [ ] IN PROGRESS
...
> **STATUS 2026-06-26.** **Spur 1 (BUILD) = COMPLETE + pushed:** all deploy-coupled Stage-B remediation (SR-1 ingress/prod-overlay, SR-6 backups+restore+monitoring+alert-delivery, SR-7 blockers + crawl/backup scheduler + provisioning hardening) is implemented, locally verified, and committed — see `.security/REMEDIATION_PLAN.md` (every BLOCKER closed; deferred tail → Phase 151). **Spur 2 (PROVISIONING) = COMPLETE:** Hetzner box live (TLS/domain/DNS/firewall), Brevo domain-authenticated, Storage Box online. **Spur 3 (DEPLOY + VERIFY) = NOT YET DONE** — the stack is not yet running on the box. The unchecked items below are Spur 3 and close the phase (+ Go/No-Go) only once executed on the real target: first `up` with TLS, first-admin bootstrap, a **tested restore from backup**, timers installed, runbook run end-to-end. Until then the phase stays open and Phase 150 (live-on-box guardrails) cannot start.
