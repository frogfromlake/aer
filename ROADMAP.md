...
> **Closed for the live deploy** (stack/TLS/auth/admin/crawl/tested-restore/timers/alerts all verified on the real box — see STATUS below). **3 unchecked items were NOT part of the live-deploy push and are carried forward** (operator rule: deliberate residuals → next phase): **transactional-email live-validation** (invite/reset *via email* end-to-end — SMTP/Brevo is proven via Grafana alert delivery, but the BFF invite/accept/reset email flow is untested) → **Phase 150**; **GHCR service-image publishing + nightly-smoke un-park** (images are built locally on the box, not pulled from GHCR) → **Phase 150**; **frontend manual passes** (Phase-128 screen-reader EN/DE + M1 fps against the deployed HEAD bundle) → **Phase 150**. The **written `deploy_runbook.md`** (the runbook executed here interactively + the 9 fresh-machine fixes) → **Phase 151**.
...
> **Carried from Phase 149** (deliberate residuals — not part of the live-deploy push): **(a) transactional-email live-validation** — invite → accept → activate + forgot/reset *via email* end-to-end (SMTP/Brevo already proven through Grafana alert delivery; only the BFF invite/reset email path is untested; bootstrap-admin used the printed link, not email); **(b) GHCR service-image publishing + nightly-smoke un-park** — publish ingestion/bff/analysis-worker/dashboard to GHCR (solve the worker model-bake disk/time once), flip `e2e_smoke_nightly.yml` to pull, rewrite `e2e_smoke_test.sh` to the web-crawler path, re-enable its cron; **(c) frontend manual passes against the deployed HEAD** — Phase-128 screen-reader EN/DE + M1 fps (globe + at-scale co-occurrence, reduced-motion). Full text in the Phase-149 items above (left unchecked there).
