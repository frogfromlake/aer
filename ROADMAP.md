...
## Phase 144: UI Localization — German (DE) [P1] - [x] DONE 2026-06-18 (all 13 UI surfaces localized + mechanism + content/WP wiring; Phase 144b closed the conceptual-vocabulary SoT tranche — DoD "no hardcoded English in any user-visible surface" fully met)
...
### Validation
* [x] Every converted surface renders DE+EN via the SideRail selector; the **parity gate** (`check-i18n-parity.mjs`, **1349/1349** after Phase 144b) + Paraglide compile fail on any en-only/de-only or stub message; `?lang=` round-trips + deep-links; date/number follow locale; WP bodies + editorial prose appear in DE; emic nouns untranslated. `make fe-lint` / `fe-test` / SSG build / bundle-size green; Go BFF content config+handler tests green. Playwright `tests/e2e/locale.spec.ts` (DE render + live switch + `?lang=de` deep-link) + a registry-driven SoT assertion in `tests/e2e/workbench.spec.ts` (`?lang=de` → View lever "Darstellung"/"Verteilung"); both run in CI. **DoD met:** with Phase 144b closing the conceptual-vocabulary SoT tranche, "no hardcoded English in any user-visible surface" is fully achieved.
