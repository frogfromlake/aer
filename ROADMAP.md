...
## Phase 148b: Pre-Deploy Readiness & Data-Quality Validation [P0] - [ ] TODO
...
* [ ] *(Optional, decided)* **Silver `raw_text` storage lever** — drop raw_text from Silver, serve the L5 "Raw" view from Bronze on-demand + a 90-day-availability UX note (~3× less storage growth). Its own focused change; see `.security/PHASE148_DECISIONS.md` §"Storage Lever #1".
...
* [ ] **Corpus-loop coordination + topic-modeling fix (148b finding, root-caused).** The BERTopic fit never completed (0 topic rows, 2 runs): a live 6→10 GiB RAM experiment proved the cap was binding but not the sole cause — the **real root is uncoordinated concurrent corpus loops** (revision-enrichment backfill + cooccurrence 1-yr sweep + topic fit pile up on independent timers → topic starves). Fix: **coordinate/stagger the heavy corpus loops** (a single sequential corpus-extractor scheduler or a mutex so topic/cooccurrence/revision-enrichment don't run concurrently) + a per-partition **timeout + heartbeat** (a stuck fit is bounded and alertable, never silently blocks the loop). Do NOT retune `n_neighbors`/`min_cluster_size`. **Pass:** a full topic tick completes on the 707-doc corpus and writes topic rows for the fr partition.
* [ ] **Worker memory sizing (148b finding).** At 707 docs the worker hit 88–91 % of its 6 GiB limit; the cooccurrence sweep loads its whole **1-year window** into RAM → grows with the corpus. Bound/stream the cooccurrence + topic windows (or batch them) so memory does not scale with total corpus, and set `compose.yaml` worker `deploy.resources.limits.memory` against the real 16 GB CX43 headroom (currently 6144M; a live `docker update` to 10 GiB during 148b confirmed headroom is needed — compose SSoT is unchanged). **Pass:** worker RSS stays bounded across a full crawl+drain; the value is justified against the 16 GB host budget.
* [ ] **Silver `raw_text` storage lever (decided in Phase 148 — feature-preserving / Bronze-on-demand).** Stop persisting `raw_text` in the Silver MinIO envelope; serve the L5 "Raw" view from Bronze on-demand. **~3× less lifetime storage growth** (Silver 0.6 MB→~0.06 MB/doc; per sustained doc/day ~288 MB→~90 MB; at 100 probes ~4 TB→~1.3 TB) — and it restores the medallion principle (Bronze = raw, Silver = refined; storing raw HTML in Silver violated it). Measured: `raw_text` is 80–96 % of every Silver object and is read by **nothing analytical** (only the L5 Raw toggle; replay/re-extraction reads Bronze). **Wiring:** (1) **Worker** — `internal/adapters/web.py:182` stops persisting `raw_text` in the serialized Silver envelope (keep it in-memory for the contract/processing). (2) **MinIO** — grant the BFF service-account **Bronze read** in `infra/minio/setup.sh` (silver-only today). (3) **BFF** — a "fetch Bronze raw by `bronze_object_key`" path wired to the L5 Raw toggle (resolve key from the Silver record → `GetObject(bronze)`); BFF stays ClickHouse+MinIO only. (4) **Evidence Reader (required UX note)** — inline note that raw source is available ~90 days (Bronze TTL), unlike cleaned text, so an empty Raw view on older articles reads as expected, not broken; new i18n key EN+DE. **Trade-off:** raw view only within the 90 d Bronze window (vs 365 d) — ample for a view-source feature, disclosed by the note. Existing Silver keeps `raw_text` until its 365 d TTL; only new writes shrink. Full rationale: `.security/PHASE148_DECISIONS.md` §"Storage Lever #1".
* [ ] **word_count body-floor policy (148b finding; decided by Task #7).** The crawler's `min_word_count` filters raw-HTML word count (a 404/stub guard), so thin pages (4–40 body words) reach Gold with a tiny `word_count`. Implement the Task-#7 over-crawl decision (enforce a body-length floor at the Silver boundary vs. disclose thin content as Negative Space) — **never silently drop** without disclosure (DISCLOSE-NEVER-COERCE).
...
* [ ] **Worker throughput + memory guardrail (verify 148c live)** — crawl-volume vs. processing-rate monitored; backlog / DLQ-growth alert; worker-RSS alert; documented response when the worker falls behind. Confirms the 148c memory sizing + loop coordination hold under real corpus growth on the box.
