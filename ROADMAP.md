...
*Three phases that mature AĒR's data-collection layer from RSS-snippet calibration to scientifically defensible full-article ingestion, and that lay the architectural foundation for the WP-003 §5 non-human-actor research question. Phase 122 retires the RSS pipeline in favour of polite, robots-respecting full-article web crawling — RSS snippets are too short for meaningful sentiment, NER, or topic modelling, and contaminate Gold-layer baselines with truncated text. Phase 123 implements the first Tier-1 stylometric authenticity extractors that operate on the new full-text corpus, giving the system a deterministic baseline for AI-generated and bot-amplified content detection. Phase 124 surfaces the authenticity signals through the existing dashboard architecture (no new surface — Solo-developer scope discipline). The iteration is positioned before Probe 1 (Phase 125) so the second probe inherits the mature pattern from day one rather than requiring a parallel migration later.*
...
* [ ] **Content catalog extension.** New entries under `services/bff-api/configs/content/{en,de}/metrics/` for each Tier-1 feature plus the optional Tier-2 score. The semantic-register description is plain-language ("How varied is the vocabulary in this article?"); the methodological-register description cites ADR-029 + WP-003 §5.2 + the `metric_validity` row.
...
* [ ] **`GET /api/v1/coverage/map`.** New endpoint. Returns per-region coverage data: `{ regions: [{ regionCode, displayName, probes: [{ probeId, language, functionsCovered: [<WP-001-function>], analyticalCapability: { ner: boolean, sentimentTier1: boolean, sentimentTier2: boolean, sentimentTier2_5: boolean, culturalCalendar: boolean, authenticity: boolean } }], functionsUnobserved: [<WP-001-function>] }] }`. Reads probe registration from PostgreSQL; reads analytical capability from the Phase 118a Capability Manifest; reads cultural-calendar presence from the manifest's `cultural_calendar` block. OpenAPI spec, `make codegen`.
* [ ] **Integration tests.** Testcontainers: with two probes registered (DE + FR), the response correctly enumerates two regions; the unobserved discourse functions (CI + SF) are explicitly listed in the `functionsUnobserved` array on both regions.
...
## Phase 126b: Per-Article Discourse Function Classification (WP-001 §5.4) [P2] - [ ] TODO

*Closes the methodological gap that Phase 122 deliberately left open. Today every Gold row inherits a single `discourse_function` tag from the source-level classification in the Postgres `sources` table. This is a known imprecision — a tagesschau sports article is not the same discourse function as a tagesschau political report, but both currently inherit `epistemic_authority` because they originate from the same source. Phase 122's deliberate scope discipline (crawl everything the source publishes; filter only on technical grounds; never on section/topic) preserved the imprecision intentionally rather than hiding it via researcher-selected URL filters per WP-006 §3. Phase 126b is the methodologically-honest complement: classify discourse function at the article level via a hybrid pipeline, fall back to the source-level classification gracefully, and surface the source-vs-article divergence on the dashboard as Negative Space (Brief §7.7). This is the canonical case of "make the imprecision visible, then measure it" rather than "filter it away to keep the dashboard tidy".*

*Scope discipline (WP-001 §5.4 + Ch.13 §13.5).* The classifier is provisional. The four discourse functions (Epistemic Authority, Power Legitimation, Cohesion & Identity, Subversion & Friction) are themselves a working taxonomy whose validation is interdisciplinary out-of-band research. Phase 126b ships the *engineering hooks* and an opinionated *first-pass classifier*; it does not claim methodological closure on the taxonomy itself. The `metric_validity` scaffold rows record the limitations honestly: zero-shot classifier accuracy degrades on adversarial paraphrasing; section-based heuristics are language- and source-specific; the four-function taxonomy is itself contested per the WP-001 working paper.
...
*Probe-pair concreteness.* This phase consumes the Probe 0 + Probe 1 data produced by Phases 125 + 126. The classifier is validated qualitatively against a small handcrafted spot-check set per language (~50 articles per probe, manually annotated by the researcher) before being released to the dashboard.

### Schema (ClickHouse)

* [ ] **`aer_gold.metrics` extension.** Two new columns:
  - `discourse_function_article LowCardinality(String)` — the per-article tag from the new classifier. Nullable; legacy rows pre-Phase-126b leave it null.
...
* [ ] **`configs/discourse_function_rules.yaml`.** Per-source URL-section rules. Initial coverage: Probe 0's two sources (tagesschau, bundesregierung) and Probe 1's two sources (francetvinfo, gouvernement) with conservative rules. Adding a new source's rules is a YAML edit, not an extractor patch — same Capability-Manifest-style discipline as Phase 118a.
* [ ] **Capability Manifest update (ADR-024).** Add `discourse_function_classification.languages` to `services/analysis-worker/configs/language_capabilities.yaml`. Languages with NLI classifier coverage today: `de`, `fr`, `en`, `es`, `it`, `ru`, `zh`, `ja`, `ar`, `hi` (mDeBERTa's pretrained set). Languages outside this set fall through to URL-heuristic + source-default.
...
* [ ] **New ADR-029: Per-Article Discourse Function Classification.** Records: (i) the four-stage hybrid classifier (manual override → URL heuristic → zero-shot NLI → source-default fallback); (ii) the rationale for not filtering at crawl time (WP-006 §3 — researcher selection bias) and instead annotating at extraction time; (iii) the dual-column ClickHouse schema preserving both source-level and article-level tags with the effective tag as a derived consensus; (iv) the classifier provisionality status (WP-001 §5.4); (v) the cross-link to Phase 122 — annotation is the methodological complement to the technical-only crawler-side filtering. (Note: ADR numbering depends on Phase-123 ADR-029 — if 123 lands first, this becomes ADR-030.)
* [ ] **WP-001 §5.4 cross-link.** Update §5.4 (Discourse Function Operationalisation) noting the per-article classifier is operationalised in Phase 126b. Add the limitations enumeration as an open research question in the Reflection hub (interdisciplinary validation invitation).
...
* [ ] **Classifier unit tests.** Five fixture articles per language (`de`, `fr`, `en` initially): clearly epistemic, clearly power_legitimation, clearly cohesion_identity, clearly subversion_friction, ambiguous → classifier should abstain. Determinism guard: identical input → byte-identical output across runs.
...
* [ ] **Schema-extension test.** Pre-Phase-126b Gold rows still queryable; new rows populate both columns; the `discourse_function` effective-tag aggregate matches the article-level tag when present, else source-level.
...
* [ ] **Append Phase 126b testing entry to `TESTING.md`.** *What to test:* the four-stage classifier pipeline (manual override → URL heuristic → zero-shot NLI → source-default), the dual-column schema, the divergence indicator on L5 Evidence, the new view-mode cell, the Negative-Space overlay extension, the manual-override workflow. *How to test:* with Probe 0 + Probe 1 data in place, trigger a re-classification (`scripts/operations/reclassify_discourse_function.py --probe probe0`); query `aer_gold.metrics` and confirm both new columns populate; switch the dashboard to the `discourse_function_divergence` view mode and confirm per-function bars render alongside the source's default tag; open an article in L5 whose URL contains `/sport/` and confirm the divergence indicator renders with `cohesion_identity` (article) vs. `epistemic_authority` (source-default); toggle Negative Space and confirm divergent articles are annotated; insert a `discourse_function_overrides` row, re-classify, and confirm the override is honoured with `discourse_function_method='manual_override'`.

### Phase ordering note

Phase 126 is a hard prerequisite — the classifier needs real multi-probe data to be qualitatively spot-checked against (single-probe spot-checks would only validate against the German rules). Phase 127 (Exploratory Composition Mode) consumes the per-article tag — without 126b, the Composition Workspace would inherit and propagate the source-level imprecision into every cross-source analysis. Phase 126b therefore sits between 126 and 127 by methodological necessity, not just numerical convenience. The phase is sized for solo-dev velocity: the classifier itself is one Python file, the schema migration is two columns plus one Postgres table, the dashboard surface reuses existing primitives (view-mode cell registration, methodology tray binding, Negative Space overlay extension). The ML-research-grade validation work (annotated corpora, precision/recall measurement, adversarial-robustness studies) is explicitly out-of-band and tracked as an open research question in the Reflection hub.
