...
## Phase 122f: Metadata Coverage Surface — Runtime Signal + Field-Level Negative Space [P2] - [ ] TODO
...
* [ ] **New ClickHouse table `aer_gold.metadata_coverage` + materialized view.** Per-source-per-field rolling aggregation of WebMeta `extraction_methods` provenance. Schema: `(source String, field String, method String, n_articles UInt64, last_seen DateTime, ingestion_version UInt64)` — `ReplacingMergeTree(ingestion_version)`, `ORDER BY (source, field, method)`. The `field` column enumerates every Tier-B/C field the WebAdapter writes; `method` is the per-field extraction-method value (`json_ld`, `open_graph`, `microdata`, `html_meta`, `heuristic_htmldate`, `derived`, or the literal string `null` for unfilled fields). The MV is fed by an `INSERT INTO ... SELECT` from the analysis worker's Silver write path; populating happens on every Silver write so the coverage matrix is always current.
* [ ] **Worker write-side update.** When the analysis worker writes Silver, also append the per-field provenance into `aer_gold.metadata_coverage`. One row per (article, field, method) tuple — a 70-article batch becomes ~1,000 rows; ClickHouse handles this trivially.
* [ ] **BFF endpoint `GET /api/v1/probes/{probeId}/metadata-coverage`.** Returns per-source-per-field-per-method counts and the derived population rate. Shape:
  ```json
  {
    "probeId": "probe0",
    "sources": [
...
* [ ] **BFF endpoint `GET /api/v1/sources/{sourceId}/metadata-coverage`.** Same shape, single-source view. Backs the dashboard's per-source dossier surface.

### Dashboard

* [ ] **Field-level Negative-Space rendering on Surface II (Brief §7.7).** When the `NegativeSpaceToggle` is active AND a field is `structurallyAbsent: true` for the active source set, the field renders with the canonical Negative-Space treatment (the methodological-register prose: "bundesregierung does not emit `author` — this is a publisher choice, not a missing observation. Tagesschau does emit it; cross-source comparisons of authorship are systematically asymmetric.") instead of the conventional empty-field placeholder. When the toggle is OFF, the field is hidden — same visual outcome as today, but the SEMANTIC distinction is now structural, not visual.
* [ ] **Probe Dossier "Metadata Coverage" panel.** A new panel in the Probe Dossier (Surface II foundation, Phase 106) that renders the per-source-per-field coverage matrix from the new endpoint. Each row is a Tier-B/C field; each cell is a per-source population rate with its `extraction_method` distribution as a tiny stacked-bar inline glyph. Cells with `structurallyAbsent: true` carry the Negative-Space marker. The panel is the user-facing realisation of `bias_assessment.md`'s table.
* [ ] **Methodology tray content binding.** Each Tier-B/C field gets a methodology-tray entry that carries the WP-003 §3.2 framing in both registers (Dual-Register, Brief §7.7) so a user landing on a low-coverage field sees the publisher-side framing, not a "we have no data" framing.

### Methodology + dossier integration

* [ ] **WP-003 update — promote §3.2 metadata-asymmetry from documentation to operationalised signal.** Add a paragraph to WP-003 §3.2 noting that the Phase-122f endpoint operationalises the asymmetry, and that AĒR analysis layers MUST consume the signal for any cross-source aggregation (no implicit imputation; absent fields are absent by design). Cite Phase 122e A13's measured bundesregierung-vs-tagesschau matrix as the empirical anchor.
* [ ] **Probe 0 dossier update.** Cross-link from `bias_assessment.md` Structural Bias #5 to `GET /api/v1/probes/probe0/metadata-coverage` and the new Surface II panel — the panel is the runtime view; the dossier is the methodological frame.
* [ ] **ADR.** New ADR — "ADR-NN: Metadata Coverage as a First-Class Runtime Signal." Records the architectural choice: per-field provenance markers on every Silver envelope feed a per-source aggregation in Gold, exposed as a typed BFF endpoint, surfaced as a field-level Negative-Space rendering on the dashboard. Cross-references WP-003 §3.2, Brief §7.7, ADR-015 (Source Adapter Protocol — already supports per-source `BiasContext`), and Phase 122e A13.

### Validation

* [ ] `make test` green including new tests for the metadata-coverage aggregation and BFF endpoint.
* [ ] `make codegen && git diff --exit-code` green — OpenAPI binding for the new endpoints is in sync.
* [ ] Manual: enable `NegativeSpaceToggle` on the Probe 0 dossier surface; bundesregierung's `author` / `articleSection` / `modified_date` cells render with the methodological-register prose, not a plain empty placeholder. tagesschau's same fields render normally.
* [ ] Manual: a cross-source aggregation that touches `author` (e.g. authorship-concentration metric) refuses to compose tagesschau + bundesregierung without surfacing the asymmetry — same gating pattern the cross-frame equivalence gate uses for normalization (Phase 115).
* [ ] WP-003 §3.2 cross-references the Phase-122f endpoint; the Probe 0 dossier cross-references the runtime panel; the new ADR records the architectural choice.
