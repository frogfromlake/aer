...
* [ ] **Full clean rebuild + run + smoke of EVERY service** — rebuild all images from scratch, `make up`, healthchecks green, run the test suites, and a manual frontend click-through of all surfaces (Atmosphäre / Workbench / Reflexion + Dossier + Account/Admin). Capture any minor frontend tweaks (acceptable to defer some post-deploy).
* [ ] **Crawler + NLP pipeline deep-check** — confirm what the crawlers and the analysis-worker actually produce under real periodic crawling; verify every retry / self-healing loop behaves (atomic A27 claim, stale-processing reaper, poison vs. transient-infra quarantine, conditional-GET/staleness refetch); inspect real Silver/Gold output quality.
* [ ] **franceinfo dataset-dominance investigation (data-quality signal)** — measured Silver counts: franceinfo **1213** / tagesschau **536** / bundesregierung **49** / elysee **24**. Determine whether franceinfo's dominance is a *crawl-config difference* (deeper sitemap/archive backfill, more discovery channels) vs *genuinely higher publication throughput* — the latter is itself a meaningful signal to surface; the former may need balancing. (Current data = 2–3 non-periodic backfill crawls.)
* [ ] **Re-measure R (docs/day) after ~1 week of periodic crawling** to firm up the Phase-148 capacity/cost model (current numbers are backfill, not steady rate).
...
### Validation
* [ ] A clean rebuild runs green end-to-end; the self-healing loops are observed working; the franceinfo-dominance question is answered (config vs. signal); the capacity model is refreshed with a real daily rate.
