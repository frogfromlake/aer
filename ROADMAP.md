...
## Phase 155: Secrets Off-Box — Deploy-Time Injection, no `.env` on the server [P1] - [x] DONE

> **✅ ENGINEERING-COMPLETE 2026-06-28 (ADR-046).** All six slices implemented + runtime-validated locally; nothing deployed (operator-gated cutover). **Slice 1** `<VAR>_FILE` everywhere — Go `pkg/secretfile` (100% cov) in the 3 viper loaders, Python `secret_files` in worker + crawler, inlined POSIX resolvers in the minio/postgres/clickhouse(+labels-load) init scripts, `_secret_lib.sh` in the 5 host scripts. **Slice 2** staging — `stage_secrets.sh` (NUL stream over SSH stdin → tmpfs `/run/aer-secrets`), `infra/secrets/secrets.manifest` (1:1 SoT), the deploy job split into checkout → **stage (gated `STAGE_SECRETS`)** → deploy. **Slice 3** compose flip — top-level `secrets:` (18) + per-service `secrets:` (12 services) + `<VAR>_FILE`, native `*_FILE` (postgres/clickhouse/minio) + Grafana `*__FILE`; local `stage_secrets_local.sh` + `make up` auto-stage. **3 runtime bugs found+fixed:** 0444 file perms (Compose ignores secret mode/uid; non-root `aer` user must read), DB_URL percent-encoding (`_secret_synth.sh` — a password with spaces broke the URL), 0444-overwrite re-stage. **Festung proven:** `docker inspect` shows only `*_FILE` paths; ingestion-api + the infra servers boot healthy from tmpfs secrets. **Slice 4** `make config-audit` (drift gate in `deploy_pull.sh`; reproduces the 2026-06-27 catch) + D9 `gen_runtime_env.sh` → secret-free `.env.runtime` via `env_file:` on the app services. **Slice 5** reboot ops — the existing `TargetDown` alert covers in-stack outages; the reboot blind-spot is operational (disable unattended-reboots + external monitor + re-stage), documented. **Slice 6** ADR-046 + arc42 §8.5.2/§11 (R-2 Resolved) + `deploy_runbook.md` Part F (operator runbook) + README + ROADMAP. **Operator cutover:** create the ~18 GitHub Environment Secrets, set `STAGE_SECRETS=true`, deploy a tag, then strip the secret lines from the box `.env` — see deploy_runbook Part F.
...
**Scope.** Audit every `.env` secret (build-time vs runtime); add `_FILE` support to the Go config (`pkg/config`) + the worker dotenv loader; convert compose to Docker secrets sourced from the external store at deploy; remove the standing `.env` from the box; retire the Phase-151 SOPS/age machinery once injection lands; re-validate backup/restore (they read secrets too); document in `deploy_runbook.md` + arc42. **Includes D9 (relocated from Phase 151, operator 2026-06-27):** replace the hand-maintained per-service compose `environment:` lists with generated per-service `env_file`s in the SAME pass — the non-secret config consolidation and the secret→Docker-secrets split are one and the same per-service wiring surface, so they are done together, not twice. **Plus `make config-audit` (operator 2026-06-27):** a cheap drift-detection check that asserts the box's effective per-service config (the worker feature flags — every `*_EXTRACTION_ENABLED` + `WAYBACK_CDX_ENABLED` true — plus security-critical flags such as `BFF_SECURE_COOKIES=true`) matches a checked-in prod manifest, failing `make preflight` on any drift. This is the DETECTION layer that complements the generated-env PREVENTION: it would have caught the **2026-06-27 incident** (`REVISION_DIFF_EXTRACTION_ENABLED` + `TOPIC_EXTRACTION_ENABLED` silently `false` on the prod `.env` — the revision/topic surfaces were empty in a live demo, with no error logged anywhere) *before* it reached production. Keep it even after the generated env lands — defense-in-depth against a manual box override or an env-generation bug.
...
### Validation
* [x] No standing secret file exists on the box (`.env` gone); secrets resolve from tmpfs `/run/secrets/*`; the boot validators stay green; a leaked `/var/lib/docker` no longer yields plaintext app secrets; backup/restore still work; `deploy_runbook.md` + arc42 reflect the injection model.
...
*Work that begins now that AĒR is live (Phase 149/150) and the architecture is closed (Phase 155). These are scientific-instrument concerns surfaced by real operation, not infrastructure.*

---

## Phase 156: Scientific Reproducibility — Corpus Snapshots, Analysis Provenance & Citable Releases [P1] - [ ] TODO

*Surfaced by the operator 2026-06-28, the first session after go-live. AĒR crawls 24/7, so the corpus continuously grows and mutates. This collides with the core promise of a scientific instrument: a **saved analysis re-runs against the live corpus** (different result later), a **published export is not reproducible** (the corpus moved on), and a researcher who needs a **specific fixed dataset cannot get one today**. This is the classic reproducibility-over-a-living-dataset problem (web archives, Common Crawl, social-media corpora) — not unique to AĒR, well-understood, and solvable. The growing corpus **is** the instrument (Aleph "the weather now" / Episteme "the climate record", ADR-035) — freezing it would defeat real-time observation. But a **specific published analysis** must point to a pinned, citable state, or AĒR is a dashboard, not an instrument. **The resolution is not either/or: the live corpus (for observation) and immutable, dated, citable "corpus snapshots / releases" (for citation) coexist** — the pattern of Common Crawl's monthly snapshots, Wikipedia's dated dumps, and Zenodo/DataCite DOIs. It aligns with AĒR's "deterministic, transparent, auditable" identity and its Negative-Space reflexivity (ADR-039). It is an **additive layer on the medallion foundation, not a rearchitecture**, and is arguably the most valuable next *methodological* feature — for a research instrument, worth more than another metric. Likely spawns a new Working Paper (WP-008, "Reproducibility & Corpus Versioning").*

**The tension, precisely.** Even a query scoped to a fixed past time-window is not stable today, because four mutation sources rewrite history retroactively:
1. **Late-arriving crawls** — rolling ~7-day discovery window; publishers backdate ~3 days, so an article enters the corpus days after its `published_date` and changes historical queries after the fact.
2. **Reprocessing** — a trafilatura upgrade replays archived Bronze → new Silver; a Tier-2 model rotation recomputes Gold; old and new rows coexist with no corpus-version flag (Phases 117/119/120).
3. **Silent edits** — publishers edit live articles; `article_revisions` (CH migration 000023, Phase 122d.0) + revision-diff / `revision_count` (Phase 133) *record that* an edit happened but do **not** version the corpus or rewind to pre-edit text.
4. **TTL purges** — a query spanning expired dates (Bronze 90 d, Silver/Gold 365 d) silently returns partial results with no "data missing before X" signal.

**What already exists (strong building blocks — do not reinvent).** Bronze-as-SoT, replayable (raw HTML verbatim; the worker `WebAdapter` re-extracts at the Silver boundary, so a trafilatura upgrade replays without re-crawl). Row idempotency: every Gold table is `ReplacingMergeTree(ingestion_version)` (UInt64 event-ns) — promising, because a query may be able to reconstruct "corpus as of version V" by watermark-filtering `ingestion_version` rather than copying data. Process-level provenance: `metric_provenance.yaml` (`extractor_version_hash`, tier, WP refs) via `GET /metrics/{name}/provenance`; Tier-2 per-row `model_hash` on `topic_assignments` (Phase 120); the Phase 119 determinism CI gate; content-catalog `contentVersion`.

**What is missing (the gap — all confirmed absent).** **(1)** No corpus-level version/snapshot concept — `saved_analyses` (PG migration 000026) persists only the opaque Workbench query `state` + name/owner/timestamps; re-open = re-run live, with no `corpus_version`/`snapshot_id`. **(2)** No provenance in exports — `cell-export.ts` `ExportPayload` carries data rows + UI metadata only, no snapshot id/hash, query, extractor-version set, or content hash; a published figure is uncitable without external bookkeeping. **(3)** No persistent identifier / citation (zero DOI infrastructure). **(4)** No long-term preservation — TTLs *delete*; a snapshotted/cited corpus must escape the ILM cycle.

**Scope (the mechanisms — what this phase builds).** A *corpus snapshot* primitive: an immutable, dated, named handle for "the corpus as analysed", recorded as metadata (timestamp, the `ingestion_version` high-watermark per Gold table, the extractor-version set in force, row counts, a content hash) — investigate the **logical-snapshot** form first (a watermark + extractor-version-set + time-window the query reconstructs over the existing ReplacingMergeTree, no data copy) before any physical copy. Analysis provenance pinning: a saved analysis (and its share) records the snapshot it ran against, so it is a reproducible `(query × corpus-version × code-version)` tuple, and warns on re-open if the live corpus has drifted. Self-describing exports: every CSV/JSON/PNG/SVG embeds a provenance manifest (snapshot id/hash, query, generation timestamp, extractor versions). Optional citable releases: a "freeze + register" path (Zenodo/DataCite DOI) for an analysis backing a publication, with the snapshotted corpus exempted from TTL. Reflexive disclosure: surface "this analysis ran on corpus-state X; the live corpus is now Y" as a tenth Negative-Space class (`negative-space.ts`).

**Open design decisions — size first, operator decides (do not pre-commit in code).**
- **Granularity / trigger:** periodic releases (e.g. monthly, Common-Crawl style) vs. snapshot-on-demand (researcher freezes their scope) vs. both.
- **Which layer to snapshot:** logical Gold watermark (cheap, query-reconstructed) vs. physical Gold copy vs. Bronze + pinned code version (fully re-derivable, heaviest). Measure the cost of each at current corpus size before choosing.
- **Citability scope:** internal pin only ("re-run gives the same numbers") vs. external DOI for published papers.
- **TTL reconciliation:** how a snapshotted/cited corpus escapes the 90 d/365 d purge (longer retention vs. cold-storage export) — the direct tension to resolve.
- **Disclosure surface:** the tenth Negative-Space class + how the Workbench/Dossier signals "live vs. snapshot".

**Grounding / preserve.** The live corpus stays the default (observation is the instrument). Reuse the existing provenance machinery (`ingestion_version`, `model_hash`, `extractor_version_hash`, `metric_provenance`) — the snapshot is their corpus-level composition, not a parallel system. Occam: prefer the logical-snapshot form over physical copies until measurement proves it insufficient. Data quality is the supreme maxim — a snapshot that silently drops the four mutation sources is worse than none. **Verify-first:** before any build, confirm against the live ClickHouse whether an `ingestion_version` watermark query genuinely reconstructs a prior corpus state (the cheap path) or whether reprocessing/late-arrival break it (forcing a heavier path).

**Validation.**
* [ ] A saved analysis re-opened after the corpus grows reproduces its original numbers when pinned to its snapshot, and visibly warns that the live corpus has drifted.
* [ ] An export carries a provenance manifest sufficient for a third party to reconstruct (or at least verify) the result; a re-export from the same snapshot is byte-identical in its data rows.
* [ ] A snapshot survives a TTL boundary that would otherwise have purged its underlying rows (or is cleanly cold-stored), and cannot be silently mutated by late crawls / reprocessing / edits.
* [ ] The methodology is written up (WP-008) and the reflexive "live vs. snapshot" disclosure ships as a Negative-Space class.

---

## Phase 157: Export Overhaul — The Unified Workbench Export & Citable Scientific Artifacts [P1] - [ ] TODO

*Operator directive 2026-06-28, framed as a top-priority quality phase: **exports are AĒR's actual output.** A cell, a panel, an analysis that a researcher exports is the artifact that leaves the system, gets put in a paper, a report, a dataset repository — and gets cited. Today export is **per-cell only** (`CellExport.svelte` + `cell-export.ts`: PNG/SVG/CSV/JSON, data rows + minimal UI metadata, no provenance). That is too narrow (no panel / multi-panel / whole-analysis export) and not scientifically self-describing (uncitable, no corpus version, no methodology, no negative-space disclosure travelling with the artifact). This phase rebuilds export into a **single generalised Workbench surface** and makes every exported artifact **publication-grade and citable**. It is tightly coupled to **Phase 156** (the provenance/corpus-snapshot layer the manifest needs) — co-design the two, sequence the manifest after 156's snapshot primitive exists.*

> **⚠️ ABSOLUTE QUALITY MANDATE.** This phase is held to the highest bar in the project. An export is the scientific output of AĒR — there is **zero tolerance** for a wrong number, a dropped row, a misleading or unlabelled axis, an unstated assumption, a missing unit, a silently-omitted Negative-Space disclosure, or a figure that cannot be reproduced. Every artifact must be (1) **honest** — it discloses what it does and does not show (ADR-039); (2) **reproducible** — it carries the Phase-156 corpus snapshot + extractor/code versions; (3) **self-describing** — a third party can understand it without the app; (4) **publication-grade** — vector figures, correct typography, no raster blur, no truncated legends, no clipped labels. "Good enough" is a defect here. Data quality + completeness is the supreme maxim ([[feedback_aer_quality]]).

**The product shape (what the operator wants).** ONE generalised Export surface in the Workbench, reachable from **every Pillar** — explicitly NOT a proliferation of per-cell / per-panel / per-multi-panel buttons. The user opens the single Export surface and chooses, along three axes:
- **Scope** — from which Pillar (one, or per-Pillar)? Within a Panel: a single cell (which one?), all cells, the whole Panel (composed), all Panels in the Window, or — **evaluate feasibility** — the **whole analysis** (all three Pillars across their Windows) as one research object.
- **Format(s)** — one or several of the supported formats (see below).
- **What to include** — figure(s), raw data, methodology / how-to-read, provenance manifest, Negative-Space disclosures, the live config — toggle-able, with citable defaults (a publication bundle includes all).

Then the user **chooses the filename and the save location themselves** (File System Access API `showSaveFilePicker` where supported; graceful download fallback elsewhere) — no app-imposed names, no silent destination.

**Evaluate scope feasibility (investigation, not assumption).** Cell export exists. Panel export must compose N cells (respecting `merged`/`split` layout + split direction). Multi-panel = a Window's panels. Whole-analysis = every Pillar × Window × Panel. Investigate the composition target for each: a multi-figure page, a paginated PDF report, or a structured bundle (ZIP). Whole-analysis is the full Research Object — measure whether it is one document or a bundle, and what it costs to render.

**Formats — evaluate existing + scientifically-relevant additions (Occam: add only what researchers actually use).** Existing: PNG, SVG, CSV, JSON. Evaluate adding — **figures:** PDF (vector, the publication standard) and possibly EPS; **data:** Parquet/Arrow (columnar science standard), TSV, XLSX (researcher-friendly), metadata-bearing CSV (CSVW); **packaging (for panel / multi-panel / whole-analysis):** a ZIP bundle structured as an **RO-Crate** (JSON-LD research-object metadata — the emerging standard) or a **Frictionless Data Package** (`datapackage.json`), containing the figures + data + a `README`/methodology + a **`CITATION.cff`** + BibTeX. Decide the minimal scientifically-complete set; do not ship a dozen formats nobody uses.

**The provenance / citability layer (the heart — ties to Phase 156).** Every non-trivial export carries a **provenance manifest**: the **corpus snapshot id + content hash (Phase 156)**, the exact scope/query + time-window, the metric(s) + their `extractor_version_hash` / `model_hash` (`metric_provenance.yaml`), the active Negative-Space classes, the AĒR app/code version, the generation timestamp, and — when minted (Phase 156 DOI path) — a DOI with a ready-to-paste citation (`CITATION.cff` + BibTeX). A bundle export is then a **self-contained, citable Research Object**: a published figure can be reproduced and verified from what travels with it.

**Data inventory — the exhaustive investigation this phase demands.** Enumerate, per presentation (all 17 in `src/lib/presentations/`), exactly what backs it: the underlying rows, the config (bins / channels / window / normalization / topN / displayLanguage), the methodology + how-to-read prose + WP anchors, the Negative-Space classes, the metric provenance. Then classify each as **mandatory** vs. **user-selectable** in an export. Nothing load-bearing (a unit, an axis definition, a disclosure, an n) may be silently dropped — if it cannot be represented in a chosen format, the export must say so, not omit it.

**UX / file handling.** A single Export entry in the Workbench chrome (reachable from every Pillar) opens an Export dialog: a **scope tree** (tick Pillars / Windows / Panels / cells), format selection, include-toggles, and a **live summary/preview of exactly what the artifact will contain** (so the user sees the provenance + disclosures before saving). Then user-chosen name + save location. Every byte that leaves is disclosed; no hidden server round-trip.

**Open design decisions — size first, operator decides (do not pre-commit in code).**
- **Bundle container:** RO-Crate vs. Frictionless Data Package vs. plain ZIP + manifest — pick by adoption + cost.
- **Which new formats actually ship:** PDF? Parquet? XLSX? — each is a real dependency/weight; measure before adding.
- **Whole-analysis shape:** one paginated PDF report vs. a structured bundle vs. both.
- **Where the Export entry lives:** SideRail (global) vs. a Window-level toolbar — must be reachable from every Pillar either way.
- **Client-only vs. server-assisted rendering:** multi-panel composition, PDF, and Parquet may need a client lib or a BFF export endpoint — measure bundle-size + dependency cost (Occam: prefer native serialisation as `cell-export.ts` already does; justify any heavy lib).

**Grounding / preserve.** The existing per-cell export keeps working — the unified surface **supersedes without regressing** it; reuse the pure serialisers in `cell-export.ts`. No new heavy dependency without measurement (a PDF or Parquet lib is a real cost — evaluate native vs. lib vs. BFF, and disclose the trade in the plan). The provenance manifest **depends on Phase 156** — co-design; the snapshot id is the spine of citability. Cross-link the Negative-Space SoT (`negative-space.ts`) so disclosures travel with the data, never just on screen.

**Validation.**
* [ ] A single Workbench Export surface, reachable from every Pillar, exports a cell, a whole Panel (merged + split), all Panels in a Window, and — if feasible — the whole analysis; no per-cell/panel button proliferation.
* [ ] The user selects scope, format(s), and what-to-include, then chooses the filename **and** save location; the dialog previews the artifact's contents (incl. provenance + disclosures) before saving.
* [ ] Every exported artifact is self-describing and citable: it carries the corpus snapshot id (Phase 156), scope/query, extractor/code versions, Negative-Space disclosures, AĒR version, timestamp, content hash, and a citation block; a re-export from the same snapshot is byte-identical in its data rows.
* [ ] Figures are publication-grade (vector PDF/SVG, no clipped labels/legends, correct units + axis definitions); no load-bearing field is silently dropped — if a format cannot carry it, the export discloses the omission.
* [ ] The full data inventory per presentation is documented; the new/added formats are justified against actual scientific use (no format bloat); any new dependency is measured and justified.
