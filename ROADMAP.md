...
**Findings (2026-06-21 full-crawl validation):**
* **[follow-up → its own fix] Topic modeling did not complete.** The de partition (19 docs) correctly yielded 0 topics (< coherent-cluster threshold); the **fr partition (392 docs) hung in the BERTopic fit for 20+ min, wrote 0 rows, and silently blocked all further topic ticks** (likely UMAP/numba cold-start under RAM pressure + concurrent revision-enrichment load — NOT a corpus-size issue, NOT the tuned params which must not be touched). Two parts: (1) safe robustness fix — wrap the per-partition fit in a timeout + heartbeat so a stuck partition can't silently block the loop and is alertable; (2) investigate the actual root cause (numba cold-start vs hang vs RAM-starvation) — likely resolved by the RAM headroom below.
* **[→ Phase 150] Worker RAM is the binding constraint.** At 707 docs the worker sat at 88–91 % of its 6 GiB limit running the full extractor suite + the cooccurrence **1-year window** + revision enrichment concurrently (no OOM yet, but the cooccurrence sweep loads the whole window into RAM → grows with the corpus → real OOM path). Levers: bound/stream the corpus-extractor windows; raise the worker mem_limit on the 16 GB CX43; stagger the heavy loops. Likely also the root of the topic hang.
* **[→ Task #7] word_count floor not enforced on extracted body.** The crawler's `min_word_count` filters on raw-HTML word count (a 404/stub guard), so thin pages (4–40 body words, e.g. tagesschau liveblog/photo stubs) pass and land in Gold. No article-body length check exists at the Silver boundary. Policy decision (drop vs disclose-as-Negative-Space vs where to enforce) → over-crawl audit in the completeness/fairness phase.

### Validation
* [x] A clean rebuild runs green end-to-end; the self-healing loops are observed working; the franceinfo-dominance question is answered (genuine throughput, ≈1.35:1 fair); the capacity model is refreshed with a real daily rate (R ≈ 101/day). *Two follow-ups recorded above (topic-modeling reliability, worker RAM → Phase 150); manual FE click-through + optional storage lever remain.*
...
* [ ] **Worker memory guardrail (found in 148b)** — at 707 docs the worker hit 88–91 % of its 6 GiB limit running the full extractor suite + the cooccurrence **1-year window** + revision enrichment concurrently. The corpus-extractor sweeps load their whole window into RAM → memory grows with the corpus → OOM path. Bound/stream the cooccurrence + topic windows (or batch them), set the worker mem_limit against the real 16 GB CX43 headroom, and add a worker-RSS alert. This is also the suspected root of the 148b topic-modeling hang.
* [ ] **Topic-modeling reliability (found in 148b — root-caused)** — the BERTopic fit did not complete (0 topic rows) across two full runs. Diagnosis isolated by a live RAM-bump experiment (6→10 GiB): (a) the 6 GiB cap WAS binding (worker crossed 6 GiB without OOM once lifted) but (b) 10 GiB did NOT fix it — the fit still ran 8+ min with no completion. **Real root cause = uncoordinated concurrent background loops.** After Wayback recovers, the revision-enrichment reattempt loop saturates CPU (700-article backfill + E5 embeddings) *concurrently* with the cooccurrence sweep (1-yr window) and the topic fit, all firing on independent timers → the heaviest (topic BERTopic) starves and never finishes. Fix: **coordinate/stagger the corpus loops** (mutex or a single sequential corpus-extractor scheduler so topic/cooccurrence/revision-enrichment don't pile up) + RAM headroom + a per-partition timeout+heartbeat (so a stuck fit is bounded + alertable). Verify on the real 16 GB box. Do NOT retune n_neighbors/min_cluster_size (a prior trial shattered the coherent FR topics).
