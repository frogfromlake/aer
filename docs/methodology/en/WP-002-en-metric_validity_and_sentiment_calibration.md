...
**Negation blindness — partially addressed (Phase 117).** The sentence *"Das ist nicht gut"* ("This is not good") receives a positive score because *"gut"* carries a positive polarity weight and the negation particle *"nicht"* is not in the lexicon. Negation handling is a solved problem in rule-based NLP (e.g., NegEx, Wiegand et al., 2010), but any rule-based negation scope detection must be validated per language. German clause structure, with its verb-final subordinate clauses, creates negation scopes that differ fundamentally from English. *Operationalisation status:* Phase 117 implements **dependency-parse-based negation scope detection** in the `SentimentExtractor` (reusing the spaCy `de_core_news_lg` model already loaded for NER). Negation cues are anchored on the `neg`/`ng` dependency relation plus a German negation-particle surface-form list (`nicht`, `kein/-e/-er/-es/-em/-en`, `niemals`, `nie`, `nirgends`, `nirgendwo`, `kaum`); polarity is inverted across the cue's syntactic subtree, bounded by clause-edge dependencies (`cc`, `mark`, `cp`, `oc`, `re`, `cj`, `cd`) so embedded clauses (`weil`, `dass`, `obwohl`) do not leak. The technical solution is in place; methodological validation per §6.2's annotation protocol is still pending.

**Compositionality and compound words — partially addressed (Phase 117).** German is an agglutinative language. The compound *"Klimaschutzpaket"* (climate protection package) is a single orthographic token that carries latent sentiment through its components — but the compound itself is not in any sentiment lexicon. Decomposition strategies (empirical compound splitting, e.g., Koehn & Knight, 2003) introduce their own error sources. Other agglutinative languages — Turkish, Finnish, Hungarian, Korean, Japanese — present analogous challenges with distinct morphological rules. *Operationalisation status:* Phase 117 integrates the deterministic frequency-list-based **`compound-split`** library (Tuggener) into the `SentimentExtractor`. For tokens unmatched by SentiWS, the extractor attempts head/tail decomposition; if both sub-words are in the lexicon, their polarities are averaged. If no valid split is found, the token remains unscored — the previous behaviour is preserved as the fall-through, so the change is non-regressive on pre-Phase-117 fixtures. The technical solution is in place; methodological validation (including the cross-language transferability of the head/tail averaging rule) is still pending §6.2's annotation studies.
