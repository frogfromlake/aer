...
**Metadata-richness asymmetry as a first-class runtime signal.** A subtler but equally consequential bias sits *below* the platform-type axis: two sources on the same platform can systematically differ in *which structured metadata fields they emit at all*. tagesschau's news pages emit JSON-LD `NewsArticle` blocks with author, modification date, and editorial section; bundesregierung's pages frequently emit none of these. Cross-source aggregations that touch authorship, revision history, or editorial labels are therefore not just sparser on one side — they are *categorically asymmetric*, because the absence is a publisher choice, not sampling variance. Phase 122f operationalises this asymmetry as a runtime signal: every `WebMeta` envelope's per-field `extraction_methods` provenance dict is rolled up into `aer_gold.metadata_coverage` and exposed via `GET /api/v1/probes/{probeId}/metadata-coverage` and `GET /api/v1/sources/{sourceId}/metadata-coverage`. A field with 0 % population over ≥ 50 articles in the trailing 30 days is flagged `structurallyAbsent: true` — the threshold that distinguishes "publisher chose not to emit" from "sampling variance". The dashboard's Probe Dossier renders the resulting matrix as a per-source-per-field grid; under the Negative Space overlay (Brief §7.7) structurally-absent cells flip from a dim placeholder to methodological-register prose. AĒR analysis layers MUST consume this signal for any cross-source aggregation that touches a Tier-B/C field — absent fields are absent by design, not by accident, and silently coercing them to zero or to a derived default would produce a metric whose comparability across sources is exactly the property WP-004 §5.3 refuses to grant without out-of-band validation.
