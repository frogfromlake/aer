...
> **Implementierungsstatus — auf Tier-1.5-Ebene GELÖST (Phase 118, 2026-05).** Die unten beschriebene Fragmentierung wird durch einen deterministischen, typgesteuerten Wikidata-Alias-Index adressiert (`scripts/build_wikidata_index.py` → `aer_gold.entity_links`). Heuristische Konfidenzgewichte (1.0 / 0.85 / 0.7 für Label / altLabel / Akzent-Fold-Treffer) sind technische Standardwerte, keine validierten Messungen — siehe Fußnote¹ zur offenen Evaluierungsarbeit, die Tier-1.5 von validiertem Entity Linking unterscheidet.
...
> ¹ Der Entity-Linking-Schritt aus Phase 118 ist explizit **Tier-1.5-heuristisch und nicht validiert**. Die Konfidenzgewichte sind festgelegte technische Standardwerte; die Tiebreaker-Regel über die Sitelink-Anzahl ist eine pragmatische Disambiguierungsregel, kein präzisions-/recall-validiertes Entscheidungsverfahren; die Typ-Bucket-Abdeckung ist auf institutionellen redaktionellen Diskurs zugeschnitten und kann Entitäten unterabdecken, die außerhalb dieses Rahmens relevant sind. Eine zukünftige Tier-2-Phase — abhängig von einer Annotationsstudie an einem Probe-0-Sample mit adjudizierten Gold-Standard-QIDs — wird Precision und Recall messen, die Konfidenzgewichte ersetzen oder neu kalibrieren und (sofern die Disambiguierungspräzision unzureichend bleibt) einen transformer-basierten Linker (BLINK, ReFinED, mGENRE oder Nachfolger) als Tier-2-Verfeinerung einführen. Bis dahin sind Ergebnisse aus `aer_gold.entity_links` als provisorisch zu behandeln; die Zeile `entity_link_confidence` in `aer_gold.metric_validity` (kontextübergreifend, `unvalidated`) regelt, wie das BFF das Feld ausliefert.
