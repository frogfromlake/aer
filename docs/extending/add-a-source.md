...
1. **Declared-inventory baseline (the denominator).** The audit CLI now emits a `completeness_baseline` block (per-channel observed inventory + a measured `suggested_floor_per_run` ≈ half the observed total), and writes that measured value straight into the suggested `expected_floor_per_run` — so the per-source floor is a *measured* starting point, not a hand-typed guess. At runtime each crawl records the publisher-declared in-window denominator per channel (`crawler_discovery_runs.declared`, with `declared_indeterminate` set when it is only a lower bound — an undatable/dateless channel, a fetch cap, or an error). `completeness = goldRows / declared` is then the figure the per-source coverage panel surfaces (`GET /sources/{id}/discovery-coverage` → `completeness`), with any indeterminate channel named as the un-captured remainder, never assumed to be 0 % or 100 %. The drift alarm (`crawler_discovery_alerts`) watches that real number from run one.
...
3. **Over-collection guard (purity).** Confirm the new source's collected documents are genuinely articles — read the per-source funnel in the coverage panel (`funnel.extractionSuccessRate` = Gold/submitted, `funnel.nonArticleRate` = thin-content/fetched). A high non-article rate is an over-collection signal — the mirror image of an under-collection gap. Thin or non-article content must be filtered *with a disclosed rule* or surfaced as Negative Space (the `collection-completeness` and `thin_content` classes), never silently counted (DISCLOSE-NEVER-COERCE).
