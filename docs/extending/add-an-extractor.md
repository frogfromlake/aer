...
## Dashboard reading content (Phase 148f / 148g)

The Workbench **Reading Guide** (the six-question "How to read" surface — `what · measure · sample · encoding · compare · limits`) is *compositional*: it explains each configuration dimension from one grounded fragment, so you never author per-combination prose. The deep **methodology** (the `MeasureDetail` block under the ladder) is rendered **per bound subject**: `cellSubjects(presentation, panel)` (`src/lib/presentations/cell-subjects.ts`) enumerates every metric/field the cell actually binds — across the single-metric slot, scatter x/y/size/colour, the metric set, the field chain, the cross-tab group-by, the co-occurrence node channels, and the facet field — and one `SubjectMethodology` block is rendered for each. So you only supply the one fragment that names *your* metric/field; it surfaces in every presentation that binds it. Where things live:

- **A new metric.** Its MEASURE prose + tier come from the catalogue entry `services/bff-api/configs/content/{en,de}/metrics/<name>.yaml` (dual register) and `metric_provenance.yaml` (tier, limitations) — both already in the table above. Nothing in the dashboard needs editing: `metricLabel`/provenance resolve by name, and the per-subject surface picks it up wherever a channel/set binds it.
- **A new metadata field** (publisher-declared structural metadata, not an analytical metric). Phase 148g (ADR-045) gave fields a **first-class `field` content-entityType**, mirroring `metric`: author `services/bff-api/configs/content/{en,de}/fields/<name>.yaml` (dual register + `displayLabel` + `workingPaperAnchors`), the same shape as a metric entry. `SubjectMethodology` fetches `GET /content/field/<name>` for a field subject. Still add the short label + one-line fallback to Paraglide (`metadata_field_label_<name>` + `metadata_field_desc_<name>` in `messages/{en,de}/metadata.json`) and register the field in `FIELD_LABELS`/`FIELD_DESCRIPTIONS` (`src/lib/state/labels.svelte.ts`) — the one-liner is the graceful fallback when no `field` content entry exists yet, and `fieldDescription` is what classifies a `Panel.metric` value as a known field.
- **A new presentation/view.** Add its `rg_*` reading fragments to `messages/{en,de}/cells.json`, a `howto_<presentation>` content-catalogue entry for its WHAT template + the per-view methodology (this is always surfaced, for every view), and — critically — classify it in **`panelSubjectKind`** (`src/lib/presentations/metric-presentation.ts`): `'metric'` if it binds a real metric in `Panel.metric`, `'field'` if it groups by a metadata field, `'none'` for channel-driven / multivariate. If the view binds metrics/fields through channels / sets / chains / a facet, add the corresponding case to **`cellSubjects`** (with role labels in `cells.json` as `rg_role_*`) so each bound subject gets its methodology block. **Per-(view × metric) pairing prose** (`<presentation>_<metric>.yaml`) is fetched ONLY for views with `usesMetric: true` (today `distribution` + `time_series`) — do NOT author pairing files for a `usesMetric: false` view; they would never be surfaced (this is why the legacy `topic_*`/`cooccurrence_*` pairing files are orphaned — ADR-045). Put view-plus-channel methodology in the `howto_<presentation>` entry instead.

The composer + types live in `src/lib/presentations/reading-guide.ts` and `cell-subjects.ts` (pure, unit-tested in `tests/unit/reading-guide.test.ts` + `tests/unit/cell-subjects.test.ts`).
