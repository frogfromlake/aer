...
- `GET /api/v1/metrics/available` — Dynamic discovery of metric names present in the Gold layer within the queried time range. **Requires `startDate`/`endDate`** (added Phase 47 — previously returned all-time metric names). Returns a JSON array of `{metricName, validationStatus, eticConstruct, equivalenceLevel}` objects (Phase 63 added `validationStatus`; Phase 65 added `eticConstruct` and `equivalenceLevel` from the metric equivalence registry — both `null` when no equivalence entry exists).
