...
- `GET /api/v1/languages` — Aggregated language detection results from the language detection pipeline. Requires `startDate`/`endDate`, supports optional `source`, `language`, and `limit` filters. Response includes `{detectedLanguage, count, avgConfidence, sources}` per detected language. Only rank-1 (top candidate) detections are included in the aggregation (added in Phase 45).
