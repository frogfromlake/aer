...
**Bias Context Propagation (Phase 64):** `SilverMeta` subclasses may include a `BiasContext` field documenting the structural biases of the source platform (WP-003). Fields: `platform_type`, `access_method`, `visibility_mechanism`, `moderation_context`, `engagement_data_available`, `account_metadata_available`. For RSS sources, these are static values reflecting the absence of algorithmic amplification and engagement data. The bias profile is written to the Silver bucket as part of `SilverEnvelope.meta` and documented in the Probe 0 dossier under `docs/probes/probe-0-de-institutional-web/bias_assessment.md` (Phase 70). Authenticity extractors (bot detection, coordination detection) are deferred to phases introducing social media adapters (see R-12).
...
- `GET /api/v1/metrics/{metricName}/provenance` — Methodological provenance for a single metric (Phase 67, WP-006). Returns `{metricName, tierClassification, algorithmDescription, knownLimitations, validationStatus, extractorVersionHash, culturalContextNotes}`. Static fields are loaded at startup from `services/bff-api/configs/metric_provenance.yaml`; dynamic fields (`validationStatus`, `culturalContextNotes`) are resolved at request time against `aer_gold.metric_validity` and `aer_gold.metric_equivalence`. Returns HTTP 404 when the metric has no provenance entry registered. Implements WP-006 Principle 1 (Methodological Transparency) — see ADR-017.
- `GET /api/v1/sources` — Canonical list of known data sources with optional documentation URLs (Phase 67, WP-006). Since Phase 87 the BFF reads this list live from the PostgreSQL `sources` table (incl. the `documentation_url` column from migration 000007) via a dedicated `bff_readonly` least-privilege role provisioned by the `postgres-init-roles` init container. An in-memory TTL cache (`BFF_SOURCES_CACHE_TTL_SECONDS`, default 60s) with stale-fallback protects Postgres under dashboard bursts — the previous static mirror config (`configs/source_documentation.yaml`) has been removed to close the source-of-truth drift gap. Phase 70 (migration 000008) repointed `documentation_url` from a single bias-profile file to the probe dossier directory `docs/probes/<probe-id>/`, so `GET /api/v1/sources` now exposes the dossier root rather than one file inside it. Implements WP-006 Principle 3 (Reflexive Documentation) — see ADR-017.
...
**PostgreSQL (Metadata Index):** The relational memory. Schema is managed via versioned SQL migrations in `infra/postgres/migrations/` executed by `golang-migrate` on `ingestion-api` startup (see ADR-014). Tables: `sources` (registered data sources with dynamic name-based lookup; Phase 67 added `documentation_url` via migration 000007 to link each source to its methodology profile; Phase 101 added `silver_eligible BOOLEAN NOT NULL DEFAULT false` plus the review-metadata columns `silver_review_reviewer`, `silver_review_date`, `silver_review_rationale`, `silver_review_reference` via migration 000011 to gate Silver-layer self-service access per WP-006 §5.2 — Probe 0's two sources are seeded as auto-eligible per Manifesto §VI), `ingestion_jobs` (job lifecycle tracking), `documents` (per-document status with unique `bronze_object_key`, `trace_id`, lifecycle states `pending` → `uploaded` → `processed` / `quarantined`, and from Phase 101 a nullable `article_id VARCHAR(64)` written by the analysis worker on commit so the BFF article-detail endpoint can resolve an article_id back to its Bronze/Silver object key), and `source_classifications` (etic/emic discourse function classifications per source, Phase 62 — composite PK `(source_id, classification_date)` enabling temporal tracking of functional transitions). Enables Progressive Disclosure by linking Gold metrics back to Bronze raw data via trace IDs.

**ClickHouse (OLAP — Gold Layer):** The high-performance analytical database. Schema is managed via versioned SQL migrations in `infra/clickhouse/migrations/`, executed by the `clickhouse-init` container on startup via `infra/clickhouse/migrate.sh` (see ADR-014). Applied versions are tracked in `aer_gold.schema_migrations`. The `aer_gold.metrics` table uses a `MergeTree` engine, ordered by `timestamp`, with a 365-day TTL. Columns: `timestamp`, `value`, `source` (data source identifier), `metric_name` (e.g., `word_count`, `sentiment_score`, `language_confidence`, `publication_hour`, `publication_weekday`, `entity_count`, `lexicon_version`), `article_id` (nullable, links back to the specific document), and `discourse_function` (Phase 62, the source's primary discourse function from the Functional Probe Taxonomy — enables aggregation by discourse function). Stores derived numerical time-series metrics inserted by the analysis worker and queried by the BFF API. The `aer_gold.entities` table (Phase 42, Migration 003) stores raw Named Entity Recognition spans: `timestamp`, `source`, `article_id`, `entity_text`, `entity_label` (PER, ORG, LOC, MISC), `start_char`, `end_char`. Uses `MergeTree` engine ordered by `(timestamp, source)` with a 365-day TTL. Entity linking is not implemented — raw spans only. The `aer_gold.language_detections` table (Phase 45, Migration 004) stores per-document language detection results: `timestamp`, `source`, `article_id`, `detected_language` (ISO 639-1 code), `confidence` (0.0–1.0), `rank` (candidate position, 1 = most likely). Uses `MergeTree` engine ordered by `(timestamp, source)` with a 365-day TTL. Storing ranked candidates preserves the full output of `detect_langs()` for downstream analysis. The `aer_gold.metric_validity` table (Phase 63, Migration 006) stores per-(metric, language, source_type, discourse_function) validation metadata used by the Hybrid Tier Architecture (ADR-016): `metric_name`, `language`, `source_type`, `discourse_function`, `alpha_score` (Krippendorff's alpha from the annotation study), `correlation` (system-vs-reference correlation), `n_annotated`, `error_taxonomy`, `validated_by`, `validation_date`, `valid_until`. Uses `ReplacingMergeTree(validation_date)` ordered by the context key. The table is initially empty — metrics without an entry surface as `unvalidated` via `GET /api/v1/metrics/available` (see §8.12). The `aer_gold.metric_baselines` table (Phase 65, Migration 007) stores per-(metric, source, language) baseline statistics for z-score normalization: `metric_name`, `source`, `language`, `baseline_value` (mean), `baseline_std` (standard deviation), `window_start`, `window_end`, `n_documents`, `compute_date`. Uses `ReplacingMergeTree(compute_date)` ordered by `(metric_name, source, language)`. Populated offline by `scripts/operations/compute_baselines.py`. The `aer_gold.metric_equivalence` table (Phase 65, Migration 008) maps etic constructs to concrete metrics with cross-cultural equivalence levels: `etic_construct`, `metric_name`, `language`, `source_type`, `equivalence_level` (`temporal`, `deviation`, `absolute`), `validated_by`, `validation_date`, `confidence`. Uses `ReplacingMergeTree(validation_date)` ordered by `(etic_construct, metric_name, language)`. Initially empty — populated when interdisciplinary validation studies establish equivalence. The `aer_gold.entity_cooccurrences` table (Phase 102, Migration 011) backs the Network Science × force-directed-graph view-mode cell defined in ADR-020. It stores per-article entity-pair co-occurrence rows emitted by the **`EntityCoOccurrenceExtractor`** — the first `CorpusExtractor` implementation in the analysis worker, run on a periodic `asyncio` loop (`CORPUS_EXTRACTION_INTERVAL_SECONDS`, default 3600 s) for every active source over a sliding `CORPUS_EXTRACTION_WINDOW_SECONDS` window. Columns: `window_start`, `window_end`, `source`, `article_id`, `entity_a_text`, `entity_a_label`, `entity_b_text`, `entity_b_label`, `cooccurrence_count` (per-article weight, `min(count_A, count_B)`), `ingestion_version`. Uses `ReplacingMergeTree(ingestion_version)` ordered by `(window_start, source, article_id, entity_a_text, entity_b_text)`, with a 365-day TTL on `window_start`. Pair ordering is canonical lexicographic so `(A, B)` and `(B, A)` collapse to one row. Idempotent: re-running the corpus sweep over the same window produces identical key tuples that ReplacingMergeTree collapses on merge / FINAL. Read by the BFF endpoint `GET /api/v1/entities/cooccurrence`, which aggregates by summing `cooccurrence_count` across articles in the requested window — the standard document-coupling network construction (Newman 2001).
...
**`pkg/middleware/`** — Shared HTTP middleware. `APIKeyAuth` validates a static API key via the `X-API-Key` header. The ingestion-api uses it as its sole gate; the BFF demotes the key to a machine credential behind its own session-OR-key gate (`internal/auth/middleware.go`, Phase 134 / ADR-040 — see Arc42 §8.7.7).
...
**Implementation:** Self-contained crawler binaries under `crawlers/`, one per platform class (ADR-028 prefers Python for new crawlers). Each crawler is independent of the service modules, preserving the "Dumb Pipes, Smart Endpoints" pattern (see ADR-010). At **runtime**, each crawler is packaged as a one-shot container gated behind the `crawlers` Compose profile and joins the internal `aer-backend` Docker network, so it reaches `ingestion-api:8081` directly without host port exposure. Crawlers are never part of `make up`; they are invoked explicitly (`make crawl`, which fans out to `crawl-probe0`/`crawl-probe1`, cron, or any host-side scheduler wrapping `docker compose run --rm <crawler>`). Communication with AĒR is still exclusively via `HTTP POST /api/v1/ingest` — the network placement is an ops detail, not a coupling.

The **Web Crawler** (`crawlers/web-crawler/`, Phase 122 / ADR-028) is AĒR's primary data source: a single configurable Python (Scrapy) binary, driven by per-probe `probes/<probe-id>/sources.yaml` config rather than per-source code. It fetches article HTML and submits it to the Ingestion Contract with `source_type: "web"`; **Bronze stores the raw HTML verbatim**, and the worker's `WebAdapter` runs trafilatura/extruct/htmldate at the Silver boundary (so trafilatura upgrades can replay archived Bronze without re-crawling). Polite defaults apply: `ROBOTSTXT_OBEY=True`, `DOWNLOAD_DELAY`, `AUTOTHROTTLE`, and conditional GETs against the PostgreSQL `crawler_state` table. Per-source discovery channels are declared in a `discovery:` block (ADR-031). See Chapter 13 and ADR-028 for the rationale.

Two earlier crawlers are retired and kept only for git-history traceability: the **legacy Go RSS Crawler** (`source_type: "rss"`, archived in Phase 122, the directory removed in Phase 129 — its `RssAdapter` remains registered in the worker so archived Bronze still replays) and the **Wikipedia Scraper** (AĒR's initial PoC, retired in Phase 43). The `wikipedia` source seed in PostgreSQL is preserved for backward compatibility with existing test data and Silver objects.
