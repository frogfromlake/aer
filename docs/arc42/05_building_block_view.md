...
- `GET /api/v1/sources` — Canonical list of known data sources with optional documentation URLs (Phase 67, WP-006). Served from a static BFF config (`configs/source_documentation.yaml`) that mirrors the PostgreSQL `sources` table plus the `documentation_url` column added in migration 000007; this keeps the BFF ClickHouse-only and avoids a direct Postgres dependency. Phase 70 (migration 000008) repointed `documentation_url` from a single bias-profile file to the probe dossier directory `docs/probes/<probe-id>/`, so `GET /api/v1/sources` now exposes the dossier root rather than one file inside it. Implements WP-006 Principle 3 (Reflexive Documentation) — see ADR-017.
