...
| **BERTopic** | Transformer-based topic modeling using embeddings + HDBSCAN clustering. | Reproducible with fixed seed and model version | Medium — embedding model is a soft dependency | **Implemented — Phase 120.** `extractors/topic_modeling.py`, written into `aer_gold.topic_assignments`. Embedding model `intfloat/multilingual-e5-large`, pinned by HuggingFace commit revision in `language_capabilities.yaml > shared.topic_modeling`; baked into the worker image at build time and run with `TRANSFORMERS_OFFLINE=1`. UMAP / HDBSCAN seeded (`random_state=42`); BERTopic is fit per detected language partition (WP-004 §3.4 — cross-language alignment is out of scope and is reserved for a future scientific workflow). `model_hash` is composed of the BERTopic version, embedding model id + revision, both seeds, and the language partition; rotating any input bumps the hash and signals topic-identity drift. Surfaced via `GET /api/v1/topics/distribution`. |
| **Multilingual BERT Sentiment** (Tier 2 default — ADR-023) | XLM-RoBERTa multilingual sentiment classifier, scoring 3-class softmax outputs to a scalar in [-1, 1]. | Reproducible with fixed seeds + pinned model revision; not bit-for-bit deterministic across CUDA backends | Medium — softmax probabilities are inspectable but the underlying transformer is opaque | **Implemented — Phase 119.** `extractors/sentiment_bert_multilingual.py`. Model: `cardiffnlp/twitter-xlm-roberta-base-sentiment`, pinned by HuggingFace commit revision in `language_capabilities.yaml > shared.multilingual_bert`. Determinism flags applied at construction (`torch.manual_seed`, `transformers.set_seed`, `torch.use_deterministic_algorithms(True)`). Language guard: only documents whose detected language is in `supported_languages` produce a metric row. Models pre-fetched at image build time; runtime stage runs `TRANSFORMERS_OFFLINE=1`. Produces `sentiment_score_bert_multilingual`. |
| **German News-Domain BERT Sentiment** (Tier 2.5 refinement — ADR-023) | German news-domain BERT classifier alongside the multilingual default; gap between the two is the WP-002 §3.2 domain-transfer signal. | Same Tier-2 reproducibility profile as the multilingual default | Medium | **Implemented — Phase 119.** `extractors/sentiment_bert_de_news.py`. Model: `mdraw/german-news-sentiment-bert`, pinned revision in `language_capabilities.yaml > languages.de.sentiment_tier2_refinement`. Strict language guard — only `core.language == 'de'` produces a row (`und` does *not* fall through). Produces `sentiment_score_bert_de_news`. The optional Tier 2.5 review-domain baseline `oliverguhr/german-sentiment-bert` is deferred per ADR-023; the methodological purpose it served is preserved by the multilingual-vs-news-domain gap. |
...
## 13.10 Probe 0: Pipeline Calibration (German Institutional Web)

> **Status:** Operational — pipeline validated end-to-end (Phase 43). Formally classified under the Functional Probe Taxonomy (Phase 62). Migrated from RSS-summary ingestion to full-article web crawling (Phase 122, ADR-028).
> **Date:** 2026-04-11 (initial classification); 2026-05-08 (web-crawl migration)

This section documents AĒR's first real data source. The source selection is explicitly **provisional** — it is driven by pragmatic engineering criteria, not by scientific probe methodology. The Manifesto's Probe Principle (§IV) requires interdisciplinary dialogue for valid probe selection; this dialogue has not yet occurred. The two sources selected here serve as **calibration data** for the pipeline, not as a scientifically representative sample of German discourse. WP-001 (§6) formally classifies Probe 0 under the Functional Probe Taxonomy: bundesregierung.de serves the **Power Legitimation** function (primary) and tagesschau.de serves the **Epistemic Authority** function (primary). Functions 3 (Cohesion & Identity) and 4 (Subversion & Friction) are entirely unrepresented. The Manifesto's Probe Principle cross-reference is unchanged by Phase 122 — that phase is a calibration improvement (full article body access, accurate publication dates, richer metadata), not a methodological rescope.

**Collection method (Phase 122 / ADR-028):** Probe 0 transitioned from RSS-feed ingestion to a generalised Python web crawler that fetches full article HTML and stores it verbatim in Bronze. The analysis worker's new `WebAdapter` runs the trafilatura + extruct + htmldate extraction pipeline at the Silver boundary, producing a five-tier `WebMeta` envelope with per-field provenance markers. The collection-method change is documented further in the Probe 0 Dossier (`docs/probes/probe-0-de-institutional-web/`).

**Formal Classification (Phase 62):** Probe 0 sources are classified in the `source_classifications` PostgreSQL table with `review_status = 'provisional_engineering'`. The `DiscourseContext` (primary function, secondary function, emic designation) propagates through both `RssMeta` (legacy) and `WebMeta` (Phase 122 onwards) into the Gold layer as the `discourse_function` column in `aer_gold.metrics` and `aer_gold.entities`. Classification `function_weights` are intentionally NULL — quantification requires the WP-001 §4.4 classification process (area expert nomination and peer review), which has not yet occurred. See `docs/templates/probe_registration_template.yaml` for the registration form.
