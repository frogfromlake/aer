...
graph TD
    subgraph "Test Pyramid"
        E2E["E2E Smoke Test<br/>(scripts/build/e2e_smoke_test.sh)<br/>Full pipeline: Ingest → NATS → Worker → CH → BFF"]
...
**E2E Smoke Test:** A bash script (`scripts/build/e2e_smoke_test.sh`) boots the entire stack via `docker compose up --build --wait`, ingests a test document, waits for pipeline processing, and queries the BFF API to verify end-to-end data flow. Currently executed manually (see Chapter 11, D-4).
...
**Coverage floor & ratchet (Phase 142 / ADR-041).** A per-scope **80% line-coverage floor** is enforced in CI for the layers where line coverage is a faithful signal: Go `internal/…` (`bff-api`, `ingestion-api`) + the `pkg` packages, Python (`analysis-worker`, `web-crawler`), and the dashboard **`$lib` `.ts`** layer. The denominator excludes thin `cmd/` mains, `main.py` entrypoints, and generated code. Two surfaces are deliberately *not* line-coverage-gated because their ecosystems do not test that way: **Svelte components** (no Vitest render path under node/jsdom — verified via extracted-`.ts` logic under the `$lib` floor + Playwright E2E + svelte-check) and the **engine-3d WebGL render core** (`engine.ts`/`GlobeControls.ts`, not unit-testable without a GL context — its math/state layer is floored, the render core is denominator-exempt and covered by visual-regression E2E). The full rationale and the binding model are recorded in **ADR-041**.
...
**Active materialized views (Phase 122c).** The three `AggregatingMergeTree` views — `aer_gold.metrics_hourly` (365 d TTL), `aer_gold.metrics_daily` (1825 d TTL), `aer_gold.metrics_monthly` (no TTL) — are live as of migration `000019_activate_metrics_resolution_views.sql`. The Phase-66 deferred-state record in `000009_metrics_resolution_views.sql` is preserved verbatim as the audit trail; activation followed the procedure that file's header documented (new migration, idempotent backfill via `INSERT INTO ... SELECT`, switch the BFF query layer). The activation criteria (p95 GetMetrics latency ≥ 1.5 s, row-cap truncation, or scans ≥ 10⁸ rows) are recorded as historical context — Phase 122's 5-year crawl horizon (Phase 122b shipped with `time_window_days = 1825`; the value was later walked back to 7 days in Phase 122e A20 for continuous-monitoring cadence, but the MV TTLs are independent of the discovery cutoff and retain the longer history regardless) materialised all three at activation time. The BFF query-layer routing lives in `Resolution.queryShape` (`services/bff-api/internal/storage/metrics_query.go`): per-resolution it returns the source table, the WHERE/SELECT bucket-column choice, and the aggregate-merge expressions (`avgMerge` / `countMerge`) needed to combine the AggregatingMergeTree state columns.
...
│   ├── probes/            # one file per probe (probe-0-de-institutional-web.yaml)
...
**Lint gate.** `scripts/build/openapi_ref_style_check.sh` (wired into `make lint` via `openapi-lint`) fails CI if any file under `services/*/api/{schemas,parameters,responses}/` contains a `$ref: '#/...'`. Path files are intentionally out of scope — the sanctioned `#/components/...` pattern must remain usable there.

**Bundling.** `scripts/build/openapi_bundle.py` produces `services/<service>/api/openapi.bundle.yaml` by emulating `kin-openapi`'s path-item flattening: it inlines path files into the root first (so their `#/components/...` refs become valid against the combined document), then resolves remaining external-file refs. The bundle is gitignored, rebuilt on demand via `make openapi-bundle`, and consumed by Swagger UI. Codegen does **not** read the bundle — it runs against the modular source directly.
...
The compact pillar payload uses short keys (`w` windows, `p` panels, `s` scopes, `pi` probeIds, `si` sourceIds, `c` composition, `v` view, `m` metric, `l` layer, `r` resolution, `n` normalization, `tN` topN, `L` locked, `lr` lockedReason, `lf` lockedFunction, `sd` splitDirection, `cc` cellControlsCollapsed, `ws` windowStart, `we` windowEnd, `mp` maximizedPanelIndex, `fi` focusedPanelIndex, `aw` activeWindowIndex) so realistic states stay well under the 8 KiB cap.

### Composition semantics

* `merged` — one Cell over the union of all ScopeGroups (BFF unions `?sourceIds=` and `?probeIds=` server-side).
* `split` — multiple Cells:
  * 1 ScopeGroup, N sources → 1 Cell per source.
  * N ScopeGroups → 1 Cell per ScopeGroup.
  * `splitDirection` controls horizontal/vertical layout.

### Configuration surface

The `ScopeEditor` modal is the sole configuration surface for Panel scope. Three entry paths:

1. **SideRail Workbench click + no pillar state** → Workbench page auto-opens ScopeEditor in create-mode, seeded from `url.selectedProbes`. Apply seeds the pillar state with one Panel.
2. **Per-Panel `[⚙ Edit scope]`** → opens ScopeEditor with that Panel's current scope; Apply commits via `updatePanel`.
3. **`+ Panel`** → opens ScopeEditor in create-mode; Apply appends a new Panel to the active Window via `addPanel(pillar, template)`.

ScopeEditor renders ScopeGroups as a stack of first-class cards (no hidden `+Compare`), exposes a per-group DF-lock dropdown that dims (but does not hide) non-matching sources, and never closes on backdrop-click or focus-loss — only Esc / Cancel / Apply close.

### Per-Panel time window

Each Panel may carry its own `windowStart` / `windowEnd`. When present they override the global `url.from` / `url.to` for that Panel only. PanelHost derives the effective window via `panel.windowStart ?? props.windowStart`; PanelControls owns the date inputs that mutate `panel.windowStart` / `panel.windowEnd` via `updatePanel`. The WorkbenchScopeBar surfaces the focused Panel's effective window mirroring PanelControls.

### Selection-State (`?selectedProbes=`)

A cross-surface shopping-cart populated by Atmosphere SHIFT-click and the `ProbeFilterModal`. Consumed by:

* Dossier — filters the catalogue to the selected probes and auto-expands them.
* Workbench — seeds the ScopeEditor's first ScopeGroup on the next open (whether via SideRail click, `+Panel`, or `[⚙ Edit scope]` on an empty-state landing).

### Cross-Language refusal vs soft banner

`composition: 'merged' AND distinct(languages in resolved scope) > 1`:

* **Episteme / Rhizome** → BFF returns HTTP 422 `cross_language_merge_unsupported` (gate enforced at the view-mode handler). Frontend RefusalSurface renders the new dual-register content entry.
* **Aleph** → no refusal. `MethodologyBanner` renders a soft cross-frame-comparability note (WP-004 §3.4). The asymmetry is intentional and documented in WP-005 §6.2 §8b.

### Caps

* `MAX_PANELS_PER_WINDOW = 8`. `+Panel` disables at the cap.
* `MAX_WINDOWS_PER_PILLAR = 4`.
* `WORKBENCH_URL_CAP_BYTES = 8192`.
* CoOccurrence multi-scope POST: 100 unique source-IDs / 25 unique probe-IDs per request → `413 scope_limit_exceeded` over-limit.

### Mutators

`src/lib/workbench/panel-mutators-pure.ts` carries the pure, testable mutators:

* `updatePanelPure(pillars, path, (prev) => next)` — commits arbitrary panel updates. No lock-protection guard (the ScopeEditor owns the lock semantic).
* `addPanelPure(pillars, pillar, template?)` — appends a new Panel; template comes from `buildPanelFromScopes` when invoked via the ScopeEditor's create-mode.
...
**Data path.** The Wikidata index build (`build_wikidata_index.py`) already parses each entity's original-case `rdfs:label` per language but lowercases it into the alias table (good for matching, wrong for display). Phase 123b additionally emits a `labels` table + a `wikidata_labels.tsv` carrying the **display-cased** label per `(QID, language)`. `wikidata-index-init` copies the TSV into the `wikidata_data` volume; a new `wikidata-labels-load` init (ClickHouse image, mirrors `clickhouse-init`) loads it into the **`aer_gold.wikidata_labels`** reference table (migration 000026, ReplacingMergeTree, no TTL — stable QID-keyed reference data). No live Wikidata calls at request time; an empty placeholder TSV is a graceful no-op until the next quarterly index rebuild populates display labels.

**Query path.** The BFF resolves labels with the **same query-time LEFT-JOIN-on-QID pattern it already uses for `wikidataQid`** (`queryNodeWikidataQids` → `queryNodeLabels` in `cooccurrence_query.go`). An optional `viewerLanguage` parameter (GET query param + POST body field) attaches a `viewerLabel` per QID-linked node and reports `linkedNodeCount` / `labeledNodeCount`. This keeps the sole internet-facing service on its single established data dependency (ClickHouse) — **no SQLite driver, no volume mount, no new attack surface** on the BFF.

**Presentation.** A per-Panel `displayLanguage` lever (`source` ↔ `viewer`, default `source`, compact URL key `dl`) is a `configurableParams` entry on `cooccurrence_network`, so `PanelControls` renders the toggle only where the cell consumes it. The **relabel target is the app content language, clamped to the index's label languages** (de/en/fr today — `viewer-language.ts`; the app is English-only for now, so relabels go to English; `APP_CONTENT_LANGUAGE` is the seam a future locale selector replaces). The cell relabels only nodes that have a `viewerLabel`, marks them (↺), leaves unlinked nodes on their source form, and always discloses the **linked-vs-unlinked coverage ratio** (caption + composed "how to read" note) so the artefact never pretends to be fully readable (WP-006 no-silent-gaps).

**Scope.** Entities + co-occurrence only. **Topics are out of scope** — a BERTopic c-TF-IDF label has no language-neutral anchor; cross-lingual topic representation is a separate open research question (`open_research_questions/phase-123b-cross-lingual-topics`). Coverage of the toggle is bounded by entity-linking (today ~12% of co-occurrence nodes carry a QID) and rises as the linker improves, with no change to this layer. Adding a display language is config-only (build `LANGUAGES` + frontend `SUPPORTED_LABEL_LANGUAGES` + one rebuild); the BFF and cell are language-agnostic (`WHERE language = ?`).
