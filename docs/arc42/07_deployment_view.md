...
The `bff-api` is the only application service that bridges both networks. Since Phase 134 (ADR-040) its gate is **session-OR-key**: browsers authenticate by the opaque `__Host-` session cookie, while `X-API-Key` is a machine credential for internal/CI callers. Only the probe endpoints (`/healthz`, `/readyz`) and the pre-auth `/auth/*` flows are unauthenticated. Traefik labels route external HTTPS traffic on `PathPrefix(/api)` to this service. See Arc42 §8.7.7 and ADR-040.
