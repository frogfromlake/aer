...
**Status update (Phase 71).** The [Scientific Operations Guide](../scientific_operations_guide.md) is now published. It documents each manual workflow end-to-end (Workflows 1–6: probe classification, validation studies, equivalence establishment, baseline computation, bias assessment, cultural calendar maintenance), each with a concrete Probe 0 walkthrough showing current status, executable commands, and outstanding collaborator dependencies. The *underlying* risk — that the scientific tables are populated only with provisional engineering defaults and that no validation study has been performed — remains until those workflows have been executed against Probe 0 by interdisciplinary collaborators.
