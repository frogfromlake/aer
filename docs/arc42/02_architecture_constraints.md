...
| **Pre-Commit: Linting** | The `scripts/hooks/pre-commit` Git hook runs **scoped** linters before every commit — only the languages the commit staged (`lint-python` for Python/ruff, `lint-go` for Go/golangci-lint + OpenAPI, `fe-lint` for the dashboard); docs-/config-only commits skip. Commits are blocked if linting fails. The full `make lint` runs at pre-push + CI. | Git hook in `scripts/hooks/pre-commit`. |
...
| **Local Dependency Auditing** | The `make audit` target mirrors the CI `dependency-audit` job locally: `govulncheck` scans all Go modules and `pip-audit` scans Python dependencies. The pre-push Git hook runs `make audit` to prevent pushing code with known vulnerabilities. Tools are installed via `make setup`, which reads pinned versions from `.tool-versions` (SSoT for developer tooling). | Pre-push hook in `scripts/hooks/pre-push`. Makefile targets `audit`, `audit-go`, `audit-python`. `.tool-versions` for version pinning. |
| **`.tool-versions` as SSoT for Tooling** | `.tool-versions` is the Single Source of Truth for developer tool versions (`golangci-lint`, `oapi-codegen`, `govulncheck`, `pip-audit`). The Makefile consumes it via `include`, CI loads it into `$GITHUB_ENV` via `grep -E '^[A-Z_]+='`. No tool version may be hardcoded in `ci.yml` or the Makefile. | `make setup` installs pinned versions. CI cache key is keyed to `hashFiles('.tool-versions')`. |
