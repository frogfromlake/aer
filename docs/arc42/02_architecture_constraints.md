...
| **Local Dependency Auditing** | The `make audit` target mirrors the CI `dependency-audit` job locally: `govulncheck` scans all Go modules and `pip-audit` scans Python dependencies. The pre-push Git hook runs `make audit` to prevent pushing code with known vulnerabilities. Tools must be installed locally (`go install golang.org/x/vuln/cmd/govulncheck@v1.1.4`, `pip install pip-audit==2.10.0`). | Pre-push hook in `scripts/hooks/pre-push`. Makefile targets `audit`, `audit-go`, `audit-python`. |
