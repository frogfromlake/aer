...
6. **Harmonization (Python):** The worker downloads the raw document from Bronze, normalizes whitespace, extracts the title, calculates the word count, and maps the result to the `SilverRecord` Pydantic model. The timestamp is extracted from the MinIO event metadata (`eventTime`), not from the system clock — this ensures deterministic, reproducible inserts.
