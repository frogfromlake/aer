...
8. **Gold Insert (Python → ClickHouse):** The **Extractor Pipeline** — an ordered list of `MetricExtractor` instances registered in `main.py` — is executed against the validated `SilverCore`. Each extractor produces zero or more `GoldMetric` results (e.g., word count produces one metric, sentiment would produce `sentiment_score` + `sentiment_subjectivity`). All metrics from all extractors are collected and batch-inserted into `aer_gold.metrics` in a single ClickHouse round-trip with columns: `timestamp` (deterministic, from event metadata), `value`, `source` (from `SilverCore.source`), `metric_name`, and `article_id` (derived from the MinIO object key). Each worker thread obtains a dedicated `clickhouse_connect` client from the `ClickHousePool` for the duration of the insert, avoiding concurrent-query errors on a shared session. A failing extractor is logged and skipped — other extractors' results are still inserted (partial extraction is acceptable; no DLQ routing for extractor failures). If the ClickHouse insert itself fails, the exception propagates, the NATS event is NAK'd for redelivery, and the PostgreSQL status remains `uploaded` — ensuring the metrics are retried.
