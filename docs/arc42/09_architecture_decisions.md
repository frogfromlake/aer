...
**Related ADRs:** ADR-020 (Frontend stack — preserved), ADR-024 (Language Capability Manifest), ADR-029 (Metadata Coverage), ADR-030 (Per-Article Discourse Function), ADR-031 (DiscoveryProtocol), ADR-034 (Multi-Panel Workbench).
...
#### Dossier (global overlay — `?dossier=open`)

Phase 123a: the Dossier is a **global overlay**, not a route — openable over any surface, fully usable without WebGL2 (`DossierOverlay.svelte`, mounted in the `(app)` layout). It is a single search/catalogue overlay (`?dossier=open`); single-probe focus rides on `?selectedProbes=` (the overlay auto-expands selected probes — there is no separate mini mode). The legacy `/dossier` route 308-redirects to the overlay grammar (`?expand=<id>` → `?dossier=open&selectedProbes=<id>`).
...
* Deep-links round-trip: `?dossier=open` and `?dossier=open&selectedProbes=a,b,c`. Metadata Coverage is a focused nested modal.

#### Workbench (`/workbench`)

AĒR's analytical surface. State tree per ADR-034: `Pillar → Window → Panel → ScopeGroup`. The page auto-opens the **ScopeEditor** in create-mode when no pillar state is present, seeded from `url.selectedProbes` when set. After Apply, the resulting Panel renders normally; Cancel leaves the user on a minimal empty state with a re-open affordance.

* `PillarSwitch` at the top — Aleph / Episteme / Rhizome tiles.
* `WorkbenchScopeBar` — reads the focused Panel's resolved scope and surfaces it as chip rows (Probes / Sources / Functions / Window). Per-Panel state, not probe-wide aggregates.
* Pillar Shell body — `WindowHost` orchestrates the Panel raster; `PanelHost` renders each Panel; the focused Panel binds `PanelControls` (Phase 122k F6 rename of `CellControls`).
* Per-Panel `[⚙ Edit scope]` button in the Panel header reopens the ScopeEditor for that Panel.
* `+Panel` opens the ScopeEditor in create-mode; Apply appends a fresh Panel to the active Window.

#### Reflection (`/reflection/*`)

Working Papers, primers, open research questions, per-metric provenance. Read-only methodology surface; no scope semantics.

---

### The Single Configuration Surface: ScopeEditor

The Workbench's scope is configured **only** through the `ScopeEditor` modal (`src/lib/components/workbench/ScopeEditor.svelte`). It is the sole entry point for:

* Single probe × N sources
* N probes × N–N sources per probe
* Probe(s) + discourse-function restriction (1–N sources within that DF)
* Multi-group composition (parallel ScopeGroups for side-by-side comparison)

Apply / Cancel are explicit. The editor does NOT close on backdrop-click or focus-loss. Esc = Cancel. ScopeGroups render as a stack of first-class cards (no hidden `+Compare` popover). Per-group DF-lock dropdown dims non-matching sources visually rather than hiding them — transparency over compactness.

Three entry paths into the ScopeEditor:

1. SideRail Workbench click + empty pillar state → page auto-opens ScopeEditor seeded from `url.selectedProbes`.
2. Per-Panel `[⚙ Edit scope]` → opens ScopeEditor for that Panel's current scope.
3. `+Panel` → opens ScopeEditor in create-mode; Apply appends a fresh Panel.
...
### Methodology Surfacing

Methodology lives in **two** places:

* **Inline accordion** under each Cell (`CellMethodology`) — default-open; carries tier badge, validation status, Dual-Register prose, provenance, working-paper anchors.
* **MethodologyBanner** (`src/lib/components/base/MethodologyBanner.svelte`) — shared primitive for soft notes (Aleph cross-language note, Episteme joint-corpus / small-corpus). Episteme + Rhizome keep their hard 422 `cross_language_merge_unsupported` refusals; Aleph uses the soft banner. The asymmetry is documented in WP-005 §6.2 §8b.

Both surfaces always link into Reflection (`/reflection/wp/wp-XXX?section=Y`) so the reader can drill from the Cell into the working paper.
...
AĒR-türkis accent throughout. Monochrome Unicode glyphs in the SideRail (no multi-color emoji). Generous whitespace in modals (ScopeEditor, ProbeFilterModal, MetadataCoverageModal). The Design Brief (`docs/design/design_brief.md`) governs tokens, typography, spacing, Epistemic Weight, Progressive Semantics, Refusal patterns — unchanged by this ADR.

---

## ADR-034: Multi-Panel Workbench State Tree (Pillar → Window → Panel → ScopeGroup)

**Status:** Accepted
**Date:** 2026-05-18 (Phase 122k baseline — supersedes prior 122i amendments)
**Related ADRs:** ADR-033 (Four-Surface Architecture), ADR-024 (Language Capability Manifest), ADR-030 (Per-Article Discourse Function).
**Related WPs:** WP-001 (Discourse Function Taxonomy), WP-004 §3.4 (Cross-Frame Comparability), WP-005 §6.2 (Joint-Corpus Topic Modeling).

---

### Decision

The Workbench state is a four-level tree, fully serialised into the URL:
...
PillarState
  ├── windows: WorkbenchWindow[] (1..MAX_WINDOWS_PER_PILLAR=4)
  └── activeWindowIndex: number

WorkbenchWindow
  ├── panels: Panel[] (1..MAX_PANELS_PER_WINDOW=8)
  ├── focusedPanelIndex: number
  └── maximizedPanelIndex?: number | null

Panel
  ├── scopes: ScopeGroup[] (1..N)
  ...
  ├── windowStart?: string         (Phase 122k F5 — per-Panel time window override)
  ├── windowEnd?: string
  ├── locked?: boolean             (DF-lock indicator)
  ├── lockedReason?: 'df_entry'
  └── lockedFunction?: string

ScopeGroup
  ├── probeIds: string[]
  └── sourceIds: string[]           (empty = whole-probe scope)
```

`url.activePillar` selects which pillar is currently rendered. The three pillar payloads are encoded as base64url JSON under `?aleph=…`, `?episteme=…`, `?rhizome=…`. Compact short keys keep the URL under the 8 KiB budget for realistic states.

---

### Composition Semantics

A Panel has 1..N ScopeGroups and a `composition` axis:

* **`composition: 'merged'`** — render ONE Cell over the union of all ScopeGroups (BFF unions probeIds + sourceIds server-side).
* **`composition: 'split'`** — render N Cells:
  * 1 ScopeGroup with multiple sources → 1 Cell per source (small-multiples).
  * N ScopeGroups → 1 Cell per ScopeGroup.
  * `splitDirection: 'horizontal' | 'vertical'` controls the layout axis.

Cells consume per-unit `ViewModeCellProps` (`scope`, `scopeId`, `sources`, `composition`, `windowStart`, `windowEnd`, `metricName`, `dataLayer`); the PanelHost translates Panel state + ScopeGroups into the appropriate per-Cell prop set.

---

### Per-Panel Time Window (Phase 122k F5)

Each Panel may carry its own `windowStart` / `windowEnd` ISO date strings. When present, they override the global `url.from` / `url.to` for that Panel only — different Panels in the same Window can analyse different time ranges. PanelHost derives the effective window via `panel.windowStart ?? props.windowStart`; PanelControls exposes the date inputs that mutate `panel.windowStart` / `panel.windowEnd` via `updatePanel`. WorkbenchScopeBar displays the focused Panel's effective window (mirroring PanelControls).

Encoding: short keys `ws` / `we` in the compact pillar payload; absent when the panel inherits the global default.

---

### DF-Lock Semantics

Panels carry a `lockedFunction` when the ScopeEditor's per-group DF-lock dropdown is set. The lock dims (but does not hide) sources that are not classified under the chosen DF for the panel's probes. The Panel's data queries still flow through the same view-mode handlers; locking is a presentation-level filter on the scope picker, not an additional refusal gate.

The Phase-122i "scope is locked, can only be re-edited by returning to the Dossier" semantic is retired. The ScopeEditor (re-openable from any Panel header) owns the lock — toggling the DF-lock dropdown is the same surface that opened the lock in the first place.

---

### Cross-Language Refusal (Episteme + Rhizome)

When `composition: 'merged'` AND the resolved scope spans multiple languages:

* **Episteme** (topic_distribution, topic_evolution) → BFF returns HTTP 422 `cross_language_merge_unsupported` (gate enforced in `view_mode_handlers.go`). Frontend renders RefusalSurface with the new dual-register content entry (`services/bff-api/configs/content/{en,de}/refusals/cross_language_merge_unsupported.yaml`). Resolution paths: narrow to one language, or switch to split composition.
* **Rhizome** (cooccurrence_network, POST `/entities/cooccurrence/query`) → same 422 refusal.
* **Aleph** (time_series, distribution) — **no** refusal. `MethodologyBanner` renders a soft cross-frame-comparability note (`WP-004 §3.4`) when `composition: 'merged'` AND `sources.length > 1`. The asymmetry is intentional: Aleph metrics (lexicon-based polarity, structural counts) are language-agnostic enough that aggregation is meaningful with caveats; Episteme + Rhizome models are language-conditioned (WP-005 §6.2).

---

### URL Grammar (Phase 122k canonical)

The only URL form for Workbench state:

```
/workbench?activePillar=<aleph|episteme|rhizome>
          &aleph=<base64url-compact-pillar-state>
          &episteme=<base64url-compact-pillar-state>
          &rhizome=<base64url-compact-pillar-state>
          &from=<ISO>&to=<ISO>
          &resolution=<5min|hourly|daily|weekly|monthly>
          &negSpace=1
          &normalization=<zscore|percentile>
          &selectedProbes=<a,b,c>
```

Phase 122h legacy flat-form URLs (`?probeId=&sourceId=&viewingMode=&view=&metric=&viewMode=&layer=`) are retired entirely — the dashboard is pre-deployment, no bookmarks exist to preserve. The reader ignores legacy params; the writer never emits them.

---

### Caps + Refusals at the URL Layer

* `MAX_PANELS_PER_WINDOW = 8`. The `+Panel` button disables at the cap.
* `MAX_WINDOWS_PER_PILLAR = 4`.
* `WORKBENCH_URL_CAP_BYTES = 8192` — hard cap on the total Workbench portion of the URL. Over-cap writes are refused at the rune-store layer and the user is offered a "discard oldest window" recovery (planned; today the write silently truncates the offending pillar).
* CoOccurrence multi-scope POST (`POST /entities/cooccurrence/query`) caps at **100 unique source-IDs** and **25 unique probe-IDs** per request; over-limit returns `413 scope_limit_exceeded` with the offending counts.

---

### Mutators

Pure mutators in `src/lib/workbench/panel-mutators-pure.ts` (testable, no rune dependency):

* `updatePanelPure` — commits an arbitrary `(prev: Panel) => Panel` mutation. The Phase-122i scope-only-lock guard is retired (the ScopeEditor owns the lock semantic now); the mutator commits scope edits on locked panels too.
* `addPanelPure(pillars, pillar, template?)` — appends a new Panel; template comes from `buildPanelFromScopes` when called via `+Panel` → ScopeEditor → Apply.
* `removePanelPure`, `focusPanelPure`, `addScopeGroupPure`, `setMaximizedPanelPure`, `toggleMaximizedPanelPure`, `addWindowPure`.
...
* `tests/unit/url-state.test.ts` — URL grammar round-trip (single canonical Phase-122k form).
* `tests/unit/url-panels.test.ts` — pillar-state base64url round-trip; rejection of out-of-bounds indices; per-Panel optional fields (resolution, normalization, splitDirection, cellControlsCollapsed, maximizedPanelIndex, windowStart, windowEnd) round-trip.
* `tests/unit/panel-mutators.test.ts` — pure mutators (add / remove / update / focus / maximize / add-window / add-scope-group).
...
## ADR-045: Per-Subject Workbench Methodology & First-Class Field Content (Phase 148g)
...
The Workbench surfaces reviewed methodology under each cell (the `MeasureDetail` block beneath the Reading Guide — ADR built on Phase 148f). Phase 148f routed that block off a single subject: `Panel.metric`. Two structural gaps followed. **(1) Multivariate / channel-driven views were unexplained.** A scatter binds up to four metrics or fields (x · y · size · colour); correlation-matrix and parallel-coordinates bind a metric *set*; cross-tab binds a group-by field × an aggregated metric; metric-lead-lag binds two metrics; the co-occurrence network binds node-size and node-colour metrics; and most views can be faceted by a metadata field. None of those bound subjects' methodology was surfaced — only the per-view "how to read" template. The product goal is "every presentation × every metric × every field is explained," which the single-`Panel.metric` model could not meet. **(2) Metadata fields had no governed methodology.** Field reading prose lived only as Paraglide one-liners (`metadata_field_desc_*`), with no dual register, no Working-Paper anchoring, no `contentVersion`/review metadata, and no CI en⇄de parity — unlike every other explained entity, which flows through the BFF Content Catalog (§8.13). A field like `author`, whose *absence* on an institutional source is a load-bearing methodological fact (WP-003 §3.2, structural absence by design), cannot be carried honestly by a one-liner.

A third, latent issue surfaced during the work: per-(view × metric) **pairing** prose (`<presentation>_<metric>.yaml`) is only ever fetched for views that bind a real single metric (`usesMetric: true`) — today `distribution` and `time_series`. The pairing files authored for `topic_distribution`, `topic_evolution`, and `cooccurrence_network` (all `usesMetric: false`) have never been surfaced since the Phase-148f `panelSubjectKind` gating, yet their existence implied coverage that did not exist.

### Decision

**1. Methodology is rendered per *bound subject*, enumerated from the panel, not from a single field.** A pure `cellSubjects(presentation, panel)` (`src/lib/presentations/cell-subjects.ts`) returns every metric/field the cell actually binds — across `Panel.metric`, the scatter channels, the metric set, the field chain, the cross-tab group-by + aggregated metric, the co-occurrence node channels, and the facet field — each tagged with the role it plays (X axis, Size, Set member, Group by, Node colour, Facet, …). `MeasureDetail` renders the per-view methodology once, then one `SubjectMethodology` block per subject (metric → dual-register content + provenance + limitations + any pairing; field → field content). Kind (metric vs field) is resolved by the consumer against the `/metrics/available` registry (`isRegisteredMetric`) + the fixed field set, since that is reactive, not pure.

**2. Fields become a first-class Content-Catalog entityType (`field`) — "Option A".** The catalogue gains `field` (OpenAPI enum + `make codegen`, `validEntityTypes`, the GET `/content/{entityType}/{entityId}` validator); field methodology is authored as `content/{en,de}/fields/<name>.yaml` with the same dual-register + `displayLabel` + `workingPaperAnchors` shape as a metric, and inherits the same startup validation, char limits, and CI en⇄de parity. The Paraglide one-liner (`metadata_field_desc_*`) is retained as the graceful fallback (and as the field-classifier), but the catalogue entry is the SoT. *(Option B — enriching the Paraglide one-liners — was rejected: it cannot carry registers, WP anchors, versioning, or the parity gate, which the absence-semantics of fields specifically require.)*

**3. Per-(view × metric) pairing prose is authored ONLY for `usesMetric: true` views.** This is now an explicit constraint, not an accident: a `usesMetric: false` view never fetches a pairing file. View-plus-channel methodology belongs in the `howto_<presentation>` entry. The orphaned `topic_*`/`cooccurrence_*` pairing files are left in place pending a delete decision; the co-occurrence node-channel substance was migrated into `howto_cooccurrence_network`.

**4. A stranded categorical field in `Panel.metric` is reconciled away on entry to a single-metric view.** `reconcilePanelForView` snaps `Panel.metric` to a real metric when the incoming value is absent from the `/metrics/available` registry (the clean metric/field separator), so a field left over from a field- or channel-driven view (e.g. `author`) cannot survive as a bogus scalar dimension. `metricSupportsPresentation` stays pure (an unknown name still defaults to the scalar presentations); the registry check lives at the reconcile site that has the registry in hand.

### Consequences

* **Positive:** "Every presentation × every metric × every field" is literally surfaced — a reader on a scatter, correlation matrix, sankey, or faceted view sees the methodology of each dimension actually bound, with its role. Fields gain the same governance (registers, WP anchors, versioning, en⇄de parity) as every other explained entity, so absence-by-design (WP-003 §3.2) can be stated honestly. The pairing constraint is explicit, so the orphaned-file trap is not re-created. The field/metric reconcile removes a real state bug.
* **Negative:** Field methodology now lives in two places — the catalogue entry (authoritative) and the Paraglide one-liner (fallback + classifier) — a small duplication kept deliberately for graceful degradation. ~40 legacy `topic_*`/`cooccurrence_*` pairing files are dead weight until the operator decides delete vs re-wire. The per-subject surface fans out N content/provenance fetches for an N-metric view (e.g. a 4-metric parallel-coordinates cell), mitigated by TanStack caching + the catalogue's 24 h `staleTime`.

## ADR-046: Secrets Off-Box — Deploy-Time Injection via Docker Secrets + tmpfs (Phase 155)
...
Until Phase 155 every credential reached the stack as a root-`600` plaintext `.env` at `/opt/aer/.env`, which Docker Compose interpolated into each service's `environment:`. Two problems followed. **(1) Secrets on disk twice.** Compose writes every `environment:` value into Docker's on-disk container config (`/var/lib/docker/containers/*/config.v2.json`) in plaintext, so a snapshotted or leaked disk yields every credential even with a hardened `.env` — this is **R-2** (§11). Encrypting the `.env` (the Phase-151 SOPS/age candidate) does not fix it: env-var secrets still land in `config.v2.json`. Only a *file*-delivered secret kept off the persistent disk closes it. **(2) Silent config drift.** A hand-transcribed prod `.env` drifts: on 2026-06-27 `REVISION_DIFF_EXTRACTION_ENABLED` + `TOPIC_EXTRACTION_ENABLED` were silently absent on the box, compose's `:-false` defaults won, and the revision/topic surfaces were empty in a live demo with no error logged anywhere.

The operator's standing instruction is that plaintext secrets must not live on the server long-term ("the `.env` must be a fortress"). Vault is over-scope for one box (Occam, ADR-044). The deployment is GitHub-Actions-driven (ADR-044 / Phase 150 CD).

### Decision

**1. Source of truth = GitHub Actions Environment Secrets; the CD job injects them at deploy.** No secret is stored on the box. The tag-triggered deploy job (`release-images.yml`) streams the secrets over SSH **stdin** (never argv, never logged; GitHub masks registered values) to `stage_secrets.sh`, gated on the repo variable `STAGE_SECRETS=true` so the change is inert until the operator opts in.

**2. Secrets live only in RAM (tmpfs).** Staging writes one file per secret into `/run/aer-secrets/` — `/run` is a tmpfs on the systemd host, so the secrets are RAM-only, gone on reboot, **never on the persistent disk**. Compose's top-level `secrets:` sources each from `${AER_SECRETS_DIR:-/run/aer-secrets}/<NAME>` and mounts it into the container's `/run/secrets/<NAME>` (also tmpfs). Naming is 1:1 across env var = GitHub secret = tmpfs file = compose secret = `/run/secrets/<NAME>`; the SoT list is `infra/secrets/secrets.manifest`.

**3. The `<VAR>_FILE` convention (backward-compatible).** Each consumer reads `<VAR>_FILE` (a path) when set, else falls back to the plain `<VAR>` — so the migration is reversible and local dev keeps working. Implemented once per runtime: a shared Go helper `pkg/secretfile` (the three viper loaders), a worker + crawler Python helper (`internal/secret_files`), and an inlined POSIX resolver in the three shell init containers. Third-party servers use their **native** support: postgres/clickhouse/minio `*_FILE`, Grafana `*__FILE` (double underscore). The host operations scripts read the tmpfs directory directly (`_secret_lib.sh`).

**4. Two non-obvious runtime constraints, fixed in the staging layer.** *(a)* Docker **Compose** (non-Swarm) ignores the secret `mode`/`uid` and bind-mounts the source file as-is, so a non-root container user (the `aer` uid in our service images) cannot read a `0600` root-owned file — the native server images read theirs as root and were unaffected, masking the issue. Fix: stage the files **`0444`** inside the `0700` directory (the directory is the host-side gate, the file is readable in-container). *(b)* `DB_URL` is **synthesised** on the box from `POSTGRES_PASSWORD` (the single Postgres secret) with the user + password **percent-encoded** (`_secret_synth.sh`), so a credential containing spaces / `@` / `:` embeds safely — the previous inline-interpolated `DB_URL` was latently broken for such passwords.

**5. D9 — non-secret config via a generated `env_file`.** `gen_runtime_env.sh` writes a secret-free `.env.runtime` (the `.env` minus every manifest secret), mounted via `env_file:` on the application services, so a new **non-secret** var is forwarded by construction rather than by remembering a `- VAR=${VAR}` line (the durable fix behind the SEC-032/036/040/043/046 "var not forwarded" class). Secrets never go here — `env_file` values would land in `config.v2.json`.

**6. `make config-audit` — drift detection (defense-in-depth).** Asserts the *running* prod stack's effective per-service env (worker `*_EXTRACTION_ENABLED` + `WAYBACK_CDX_ENABLED` true, `BFF_SECURE_COOKIES=true`, …) against `infra/config/prod_config_audit.manifest`; run automatically by `deploy_pull.sh` after every deploy (drift fails the deploy) and available as `make config-audit`. It is the detection layer that would have caught the 2026-06-27 incident before a demo.

**7. Reboot recovery = pure fortress + operational downtime-minimisation.** Zero standing secrets means a reboot leaves the stack down until the operator re-stages (re-run the deploy). The trade-off is mitigated operationally: disable unattended reboots (reboots become operator-initiated, so re-staging is expected), an **external** uptime monitor for the reboot blind-spot (the in-stack `TargetDown` alert cannot self-deliver when Grafana — which itself needs a secret — is down), and a one-command re-stage (re-run the deploy job). A TPM-sealed `systemd-creds` auto-heal is a possible later enhancement, deferred (depends on the host exposing a vTPM).

### Consequences

* **Positive:** No plaintext secret on the box's persistent disk — `docker inspect` / `config.v2.json` show only `*_FILE=<path>`. **R-2 is resolved.** Boot-time secret validation, least-privilege roles, and the no-secret-in-image rule are all preserved (the file decrypts *into* the same env surface, it does not fork it). A new non-secret var forwards by construction (D9); drift is caught at deploy (config-audit). Local dev keeps a single `.env` source — `make up` auto-stages `./.aer-secrets` + `.env.runtime` (gitignored), so the dev/prod wiring is identical.
* **Negative:** A reboot now requires a manual re-stage (the deliberate fortress trade-off). The local dev `make up` gained an auto-staging prerequisite (hidden in the Makefile). Optional email secrets (`SMTP_PASSWORD`, `GF_SMTP_PASSWORD`) are staged as empty files when unconfigured so `secrets: file:` always resolves. `RESTIC_PASSWORD` must stay escrowed off-box (a password manager) — losing it makes backups unrecoverable, by design.
