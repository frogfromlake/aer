...
- [x] `src/lib/components/evidence/L5EvidenceReader.svelte` — **1286 → 395** · **DONE (Phase 141, 2026-06-16).** Tier-2b net (operator decision): no cheap render seed exists — L5 is only reachable via a 3-level drill-down (Workbench → revision/cooccurrence cell → `ArticleListModal` → row → L5) across 4 un-refactored components, vitest is node-only, and no existing e2e opens an article modal — so svelte-check + careful review + git is the markup net, strengthened by lifting the genuinely-complex (previously inline + UNtested) editorial-walk/diff-chain logic + formatters → **`l5-evidence-internals.ts`** (`deriveDiffChain`/`selectedDiffPairIndex`/`silentEditSignals`/`nsMarkersFor`/`formatTs`/`sentimentArrow`/`fmtDelta`/`wordDiff`/`walkStepLabel`/`cumulativeLabel`/`lookupStatusLabel`) + **29 unit tests** (suite 366→395). Per-region children: **`L5MetaGrid`** (84, Silver metadata header) · **`L5NegativeSpaceSection`** (89, owns silent-edit signal derivation + nsSectionOpen $state/$effect) · **`L5DiffTab`** (513, owns the diffQ TanStack query + diffView/walkPos $state + re-seed $effect + all diff-* CSS) · **`L5RevisionHistory`** (244, refusal+success branches + per-capture rows + discourse-shift deltas). Parent is now a 395-LOC orchestrator owning the 2 independent article queries (detailQ + revisionsQ) + the Article-body tab + tab strip; shared `.toggle-btn`/`.muted`/`.error`/`.provenance-summary` duplicated per child (Svelte per-component scope). Allowlist entry removed (ratchet locks it). svelte-check + fe-lint(+knip) + fe-test (395) green · evidence · **M** · 500
...
- [x] `src/lib/components/workbench/ScopeEditor.svelte` — **997 → 457** · **DONE (Phase 141, 2026-06-16).** EXTEND-THE-NET-FIRST (reachable from the Workbench, unlike L5): the `tests/e2e/workbench.spec.ts` net was extended with a ScopeEditor describe block (⚙ Edit scope → edit-mode dialog + seed ScopeGroup card + checked probe chip + 2 source rows; ＋ Panel → create-mode dialog + Apply-disabled + Add-scope-group → 2nd card) run green BEFORE refactoring. Pure draft-mutation cores lifted → **`scope-editor-internals.ts`** (`DISCOURSE_FUNCTIONS`/`sourceMatchesDf`/`toggleProbeInGroup`/`toggleSourceInGroup`/`selectAllSourcesInGroup`/`clearSourcesInGroup`/`pruneSourcesToLock`/`resolvePanelLock`) + **20 unit tests** (suite 395→415). Per-region children: **`ScopeGroupCard`** (341, one first-class group card — header + step 1 Probes + step 2 DF chips) · **`ScopeGroupSources`** (300, step 3 — the per-probe source picker, split out so the card stays <530). Parent is now a 457-LOC orchestrator owning the draft `$state`, the internal probesQuery + per-probe dossier cache/effect, and thin mutators delegating to the internals. Allowlist entry removed (ratchet locks it). svelte-check + fe-lint(+knip) + fe-test (415) + workbench e2e (6) green · workbench · **M** · 500
- [x] `src/lib/components/account/AnalysesOverlay.svelte` — **974 → 489** · **DONE (Phase 141, 2026-06-16).** EXTEND-THE-NET-FIRST (the overlay is mounted globally in the (app) layout, opened by `?analyses=open` over the root Atmosphere page — no pillar seed needed): a new `tests/e2e/analyses.spec.ts` (4 tests — toolbar + filter fieldsets + sortable header + rows + access badges, live search-filter narrowing, owned-row drawer with edit/share/delete + existing shares, read-only drawer omitting them) run green BEFORE refactoring (same auth gotcha: catch-all `**/api/v1/**`→200 first, then `/auth/me` + `/analyses` + `/shares` shaped routes). Pure client-side filter/sort/format + deep-link logic lifted → **`analyses-overlay-internals.ts`** (`filterAnalyses`/`sortAnalyses`/`nextSort`/`sortArrow`/`fmtDate`/`stripDeepLink`/`isSaveableWorkbenchUrl`/`findEditableLoaded`) + **19 unit tests** (suite 415→434). Per-region children: **`AnalysisTable`** (183, the loading/error/empty/thead/tbody list — pure presentation, emits row-open + sort-toggle intents) · **`AnalysisDrawer`** (393, the detail/share/delete master-detail pane — owns its own edit/share `$state` + API calls + `loadAnalysis`, re-seeds via a `$effect` on the `analysis` prop). Per-REGION not per-row (the rows carry no individual actions, so a whole-table child is the cohesive region; technique #7). Parent is now a 489-LOC orchestrator owning the list + save `$state`, the save flow (via the internals' deep-link helpers), and `selectedId`, passing resolved data as props. Allowlist entry removed (ratchet locks it). svelte-check + fe-lint(+knip) + fe-test (434) + analyses e2e (4) green · account · **M** · 500
...
- [x] `src/lib/components/workbench/CellConfigPopover.svelte` — **660 → 223** · **DONE (Phase 141, 2026-06-16).** EXTEND-THE-NET-FIRST and cheap: the popover is reachable from the existing `tests/e2e/workbench.spec.ts` distribution seed (Split → 2 source cells, each shows ⚙ Cell since `perCellConfig` = path≠null ∧ >1 cell ∧ `configurableParams>0`), and distribution declares `bins`+`scales`+(usesMetric) a dimension peek — so ONE cheap seed exercises all THREE popover control primitives (select / slider / switch) **and** the override→dot→custom-note→Reset lifecycle (toggling Scale creates/clears an override deterministically); no costly cooccurrence/scatter d3 seed needed (those lever branches are structurally identical markup). A CellConfigPopover describe block (2 tests) was run green BEFORE refactoring. Pure clamps + option-builders lifted → **`cell-config-popover-internals.ts`** (`clampBins`/`clampTopN`/`clampForce`/`axisOverrideForcesFree`/`buildAxisMetricOptions`/`buildDimensionOptions`) + **15 unit tests** (suite 434→449). **Order-preserving** 2-child split (boundary after `scales`, before `networkChannels` — preserves the popover's lever order for every presentation): **`CellConfigValueLevers`** (348, dimension peek + Bins/TopN/Spread sliders + Band/Scale switches — owns its own override `$state`/handlers) · **`CellConfigChannelLevers`** (266, network Size/Colour selects + display-language switch + scatter X/Y/Size/Colour grid). Parent is now a 223-LOC shell (dialog + inherit/custom note + Reset + Esc). **Did NOT reuse `levers/LeverRow`+`LeverButton`** (the memory's hypothesis): the popover is a deliberately compact `.ccp-*` card (Phase-126 overflow fixes: 100%-max selects; per-row override dots `●` the strip eyebrow can't carry) — reusing the roomier `.ctrl-*` strip primitives would have been a visual regression, not a behaviour-preserving decomposition (technique #7 — don't force primitives where the markup doesn't repeat *as the same visual*). The small `.ccp-row/.ccp-rk/.ccp-dot/.ccp-btn/.ccp-select` CSS is duplicated per child instead (PanelHost/L5 precedent). Allowlist entry removed (ratchet locks it). svelte-check + fe-lint(+knip) + fe-test (449) + workbench e2e (8) green · workbench · **M** · 500
- [x] `src/routes/(app)/reflection/wp/[id]/+page.svelte` — **642 → 91** · **DONE (Phase 141, 2026-06-16).** EXTEND-THE-NET-FIRST and cheap: the page is a client-side load (`ssr=false`, served via the SPA `index.html` fallback) that fetches `/content/papers/{id}.md` and renders it; `getPaperMeta` is a local table (wp-001 exists), so a new `tests/e2e/reflection-wp.spec.ts` (2 tests — breadcrumb/TOC/section headings; the `?from=workbench` Back-to-Workbench referrer link) mocks just the markdown fetch + auth and runs green BEFORE refactoring. CSS-DOMINATED giant (script 78 / markup 163 / scoped-CSS 471), so per-REGION children carrying their own CSS: **`WpBreadcrumb`** (102 — ScopeBar breadcrumb + the referrer→Back-to-Workbench logic) · **`WpTableOfContents`** (174 — the absence-margin + TOC list; the always-on `.neg-space .toc` accent folded into the base `.toc` since the Phase-112 toggle was retired, eliminating a cross-component selector) · **`WpPaperBody`** (397 — header + sections + footer + the `.prose :global(*)` typography). Pure transforms lifted → **`wp-page-internals.ts`** (`splitSections`/`scrollTargetIds`/`buildBackToWorkbenchHref`) + **7 unit tests** (suite 449→456). Parent is now a 91-LOC shell (data→derived wiring + `?section=` scroll onMount + layout grid). Allowlist entry removed. svelte-check + fe-lint(+knip) + fe-test (456) + reflection-wp e2e (2) green · routes · **S** · 500
...
- [x] `src/lib/components/dossier/ProbeCard.svelte` — **568 → 381** · **DONE (Phase 141, 2026-06-16).** EXTEND-THE-NET-FIRST and cheap: ProbeCard renders in the global Dossier overlay (mounted in the (app) layout, opened by `?dossier=open`), and a probe carried in `?selectedProbes=` auto-expands its card — so a new `tests/e2e/dossier.spec.ts` (2 tests — header/capability-matrix/DF-cards covered-vs-uncovered; the per-card expand revealing its source grid) seeds `/?dossier=open&selectedProbes=<id>` over `/` (no pillar state) and runs green BEFORE refactoring. The bulk + the real logic is the Discourse-Function region → child **`ProbeDfCards`** (244 — the four collapsable DF containers + unclassified, owns the per-DF expand `$state`; df-* CSS moves with it). Pure grouping/coverage lifted → **`probe-card-internals.ts`** (`groupSourcesByFunction`/`unclassifiedSources`/`coveredFunctions`/`publicationRatePerDay`/`coveragePercent`) + **8 unit tests** (suite 456→464). Parent is now a 381-LOC shell (queries + header + structural meta + capability matrix). **Note:** the capability-matrix-only split the census originally hypothesised was too small (~28 LOC → still >530); the DF-region split is the correct cohesive cut. Allowlist entry removed. svelte-check + fe-lint(+knip) + fe-test (464) + dossier e2e (2) green · dossier · **S** · 500
