...
> **Cross-reference.** The structured `BiasContext` values below are emitted into every Silver record by `RssAdapter` (`services/analysis-worker/internal/adapters/rss.py`). The Scientific Operations Guide → **Workflow 5: Assessing Bias for a Data Source** (Phase 71) describes how these values are produced for new sources.
