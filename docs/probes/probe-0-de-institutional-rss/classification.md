...
> **Status: `provisional_engineering`.** WP-001 §4.4 Steps 1–2 (area expert nomination, peer review) have not been executed. `function_weights = NULL` until that process produces quantified shares. See Scientific Operations Guide → Workflow 1 (Phase 71) for the path from `provisional_engineering` to `reviewed`.
...
- Scientific Operations Guide → **Workflow 1: Classifying a New Probe** (Phase 71)
- Operations Playbook → **`source_classifications` table** section
