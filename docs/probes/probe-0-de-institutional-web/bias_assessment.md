...
8. **Discovery-Surface Asymmetry Between Sources (specific instances closed by Phase 122g; methodological category retained).** **(New, Phase 122e A15 / F-A15; refined Phase 122e A20; specific instances closed by Phase 122g; methodological category retained as an ongoing concern.)** Each publisher chooses which discovery surfaces (sitemap, RSS feeds, HTML navigation pages, date-indexed archive walkers, robots.txt directives) to expose to crawlers. AĒR ingests what the publisher exposes — by design, per the Manifesto's "unaltered mirror" principle and WP-006 §3's prohibition on researcher-side editorial gating.

   **Resolution status (Phase 122g, 2026-05-12).** The specific instances of asymmetry on Probe 0's two sources are closed by Phase 122g's per-source `discovery:` block (ADR-031):

   * **tagesschau** — sitemap unchanged (publisher has no XML sitemap), `rss_hint_urls` unchanged (one publisher-curated main feed), `html_sitemap_urls` activated for `/infoservices/startseite-sitemap-102.html` (≈ 60 article links/day), `archive_index` activated for `/archiv?datum=YYYY-MM-DD` (≈ 30–40 articles/day × 7 days = ≈ 250 unique articles per crawl run after URL-union dedup with RSS + HTML sitemap).
   * **bundesregierung** — sitemap unchanged (three publisher-declared `sitemap_index.xml` URLs from robots.txt), `rss_hint_urls` extended from 1 → 4 feeds (the publisher's full RSS catalogue from `/breg-de/service/newsletter-und-abos/rss-newsfeed`).

   **The asymmetry as a methodological category remains a recorded structural-bias dimension.** Future sources may have entirely different discovery surfaces; Phase 122g's per-channel coverage telemetry (`crawler_discovery_runs` Postgres table, `GET /api/v1/sources/{id}/discovery-coverage` endpoint, dashboard `DiscoveryCoveragePanel`) catches new asymmetries as they emerge — the two-consecutive-runs underflow gate surfaces a `discovery_underflow` event when a source's per-run URL count falls below the configured `expected_floor_per_run`. The pre-Phase-122g "closed by continuous-mode operation" framing was overly optimistic for high-volume publishers (tagesschau's RSS-only configuration captured ≈ 1 day of the publisher's output against the configured 7-day window — gap visible only via post-hoc inspection); the Phase 122g architecture makes the gap observable at runtime.
...
4. **Publication Frequency.** Approximately 60 articles per day published across all topic areas (operator-observed empirical rate, cross-checked against the publisher's own daily HTML sitemap surface at `/infoservices/startseite-sitemap-102.html`). Earlier dossier estimates of "≈ 50" and the speculative "150–300/day" range were incorrect — corrected in Phase 122g. The discoverable surface per crawl run is bounded by the channels the publisher exposes; with Phase 122g's `html_sitemap_urls` + `archive_index` activation the configured channels together cover the publication firehose. tagesschau.de remains the dominant volume contributor in Probe 0, which must be accounted for in any cross-source comparison.
